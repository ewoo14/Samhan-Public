...
const SCREENSHOTS_DIR = path.resolve(_dirname, '../../../../docs/qa/ecount-fold-slice1')
...
test('MASTER — 회계 관리자 그룹에 잔액 스냅샷 미노출 + 형제 항목 유지', async ({ page }) => {
  ...
  // 형제 항목은 유지(silo 폐기는 aging 한정)
  await expect(page.getByTestId('sidebar-accounting-admin-cash-disbursements'), '지출 트랜잭션 유지').toBeVisible()
