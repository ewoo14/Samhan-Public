 ...
 *
 *   05-estimate-set-components-spec.png — 싱글세트 구성품 사양 모달(실내기/실외기/판넬/리모컨 + 합산 사양)
 *   06-estimate-option-delta.png        — 싱글세트 표 선택(qty=1) → 부품행 실내/외 배분가 +
...
try {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // #5 — 싱글세트 구성품 사양 모달 (실내/외 물리치수 + 판넬/리모컨 사양 합산)
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // #6 — 싱글세트 표 선택(qty=1) → '보기'로 부품행(실내/외 배분가) 펼침 + 판넬 옵션 변경 동적 금액차
  // ─────────────────────────────────────────────────────────────────────────
  // (0) '싱글세트' 탭으로 전환 (기본 진입은 전표작성 카드라 표가 안 보임).
