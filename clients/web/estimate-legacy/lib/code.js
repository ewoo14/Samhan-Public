 ...
 *  - 외부 호출 함수 15개 → async (UrlFetchApp.fetch await)
...
// 노션 인증 데이터베이스
const AUTH_TOKEN = (process.env.NOTION_TOKEN_AUTH || '__MISSING_NOTION_TOKEN_AUTH__');
...
async function doGet() {
  ...
  const authResult = await checkUserAuth(userEmail);
  t.authData = JSON.stringify(authResult);
  ...
  // t.customers       = JSON.stringify(getCustomers_());

  // 추천
  t.recommendData   = JSON.stringify(getRecommendOduData());

  // 스펙 상세맵
  t.specDetailMap   = JSON.stringify(getSpecDetailMap_());

  // 인상가 할당
  t.priceInc = JSON.stringify(getPriceIncData_());
...
// 로그인
async function getEcountSession(authInfo){
  Logger.log('🔑 세션생성');
...
async function sendOrderFromUi(data) {
  try {
    ...
    const { sessionId, zone } = await getEcountSession(authInfo);
...
// 전표발송 내역 Notion
const NOTION_KEY_SEND = (process.env.NOTION_TOKEN_SHIPPING || process.env.NOTION_TOKEN_SEND || '__MISSING_NOTION_TOKEN_SHIPPING__');
const NOTION_DB_SEND = '2f8a1006d658803face6fdfe2b175780';

// 노션전송
async function saveOrderToNotion(info, items, slipNo) {
  ...
  // 전송
  try {
    const res = await UrlFetchApp.fetch('https://api.notion.com/v1/pages', options);
...
async function logFrontEvent(group, msg, isMobile, mgrName) {
  ...
  try {
    await UrlFetchApp.fetch('https://api.notion.com/v1/pages', {
      method: 'post',
      headers: {
        'Authorization': 'Bearer ' + (process.env.NOTION_TOKEN_DC || '__MISSING_NOTION_TOKEN_BEARER__'),
        'Notion-Version': '2022-06-28',
        'Content-Type': 'application/json'
      },
      payload: JSON.stringify(payload),
...
// 재고테이블생성
async function getInventoryTableHtml(baseDate, itemCodes) {
  
  try {
    // 세션획득
    const session = await getEcountSession(); 
...
async function getInventoryTable(dateVal, itemCodes) {
  ...
  return await getInventoryTableHtml(dateVal, itemCodes);
}
...
/* 노션 설정 */
const NOTION_TOKEN_QUOTE = (process.env.NOTION_TOKEN_QUOTE || '__MISSING_NOTION_TOKEN_QUOTE__');
