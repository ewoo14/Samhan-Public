 ...
 *  - 외부 호출 함수 21개 → async (UrlFetchApp.fetch await)
...
async function doGet() {
  ...
  t.homemulti       = JSON.stringify(getHomeMulti());
  t.singleSets      = JSON.stringify(getSingleSets());
  t.singleParts     = JSON.stringify(getSingleParts());
  t.homeDefaults    = JSON.stringify(getHomeDefaults());
  t.singleDefaults  = JSON.stringify(getSingleDefaults());
  t.singleMatPrices = JSON.stringify(getSingleMatPrices());
  t.commercialMulti = JSON.stringify(getCommercialMulti());
  t.commercialParts = JSON.stringify(getCommercialParts());
  t.oldProducts     = JSON.stringify(getOldProducts_());

  t.homeInc = JSON.stringify(getHomeIncreasePrices_());
  t.commInc = JSON.stringify(getCommIncreasePrices_());
  t.singleInc = JSON.stringify(getSingleIncreasePrices_());
  t.singlePartsInc = JSON.stringify(getSinglePartsIncreasePrices_());

  // 스펙상세맵
  t.specDetailMap   = JSON.stringify(getSpecDetailMap_());
...
var NOTION_TOKEN = (process.env.NOTION_TOKEN_DC || '__MISSING_NOTION_TOKEN_DC__');
var NOTION_VER   = '2025-09-03';
const NOTION_TOKEN_AUTH = (process.env.NOTION_TOKEN_AUTH || '__MISSING_NOTION_TOKEN_AUTH__');
const NOTION_DB_ID_AUTH = '2dda1006d6588047b1bbc7c2660203c0';
const NOTION_TOKEN_ORDER = (process.env.NOTION_TOKEN_ORDER || '__MISSING_NOTION_TOKEN_ORDER__');
const NOTION_DB_ID_ORDER = '2eca1006d65880109d91c2e56fab28f4';
const NOTION_TOKEN_SNAPSHOT = (process.env.NOTION_TOKEN_SNAPSHOT || '__MISSING_NOTION_TOKEN_SNAPSHOT__');
const NOTION_DB_ID_SNAPSHOT = '33aa1006d6588087810ffaa7dc7f315c';

// 주문저장
async function saveOrderSnapshot(payload) {
  try {
    ...
    await UrlFetchApp.fetch('https://api.notion.com/v1/pages', options);
    Logger.log('💾 주문저장 완료');
    return true;
...
// 주문 내역조회
async function getOrderSnapshotHistory(bizNo, sDate, eDate) {
  try {
    const filters = [
...
// 조회
async function callZoneApi(comCode){
  Logger.log('🌐 조회');
  const res = await UrlFetchApp.fetch('http://152.69.228.109:3000/proxy/ecount/zone',{
...
// 로그인
async function getEcountSession(){
  Logger.log('🔑 세션발급');
  ...
  const zoneRaw = await callZoneApi(COM_CODE);
  const zone = String(zoneRaw).toLowerCase();

  const res = await UrlFetchApp.fetch('http://152.69.228.109:3000/proxy/ecount/login',{
    method:'post', contentType:'application/json',
...
// 주문 전송
async function sendOrderFromUi(items, order){
  try{
    ...
    // 주문 전송 시도 알림
    await logActionToNotion(logKey, logName, `주문 전송 완료 (총 ${items.length}개 품목) ${deviceTag}`);

    // 이카운트 세션
    const { sessionId, zone } = await getEcountSession();
    // 날짜/창고
    ...
    // DC 설정(노션 + 기본값)
    const dcCfg = await initDcConfigFromNotion(key);
    ...
    if (ok) {
      ...
      try {
        ...
        // 저장 함수 호출
        await saveOrderToNotion(notionCommon, merged, extractedIoNo);
        
        Logger.log('>> ✅ 노션 저장 완료 (전표번호: ' + extractedIoNo + ')');

        await logActionToNotion(logKey, logName, `주문 성공 ${deviceTag} - 전표번호: ${extractedIoNo}, 금액합계: ${totalAmt.toLocaleString()}원`);
      } catch (e) {
        ...
        await logActionToNotion(logKey, logName, `주문 실패 ${deviceTag} (이카운트 거부): ${errText}`);
    }

    return { ok, status:code, body, sentCount: SaleOrderList.length };
  }catch(e){
    await logActionToNotion(logKey, logName, `주문 처리 중 시스템 에러 ${deviceTag}: ` + e);
...
// 노션에서 거래처별 DC 설정 조회
async function fetchNotionDcConfig_(biznoDigits, forceRefresh) {
  // 숫자만 남김
  ...
  try {
    ...
    if (NOTION_VER === '2025-09-03') {
      try {
        ...
        var dbRes = await UrlFetchApp.fetch(dbUrl, {
          method: 'get',
          headers: headers,
          muteHttpExceptions: true
        });
...
async function initDcConfigFromNotion(bizno) {
  ...
  var notion = await fetchNotionDcConfig_(raw);
  if (notion) {
    ...
    await logActionToNotion(raw, cName, logMsg);
    Logger.log('>> ⚙️적용 ' + raw);
  } else {
    Logger.log('>> ⚠️없음 ' + raw);
    await logActionToNotion(raw, cName, '기본할인 적용');
  }
...
// 승인 요청 (미승인 -> 요청)
async function requestAuthApproval(bizNo, isMobile) {
  const cleanBiz = String(bizNo).replace(/[^\d]/g, '');
  // 거래처명 조회
  const found = searchCustomerByBizOrCode(cleanBiz);
  ...
  try {
    await createAuthRow_(cleanBiz, name);
    await logActionToNotion(cleanBiz, name, `최초 승인 요청 (비밀번호 설정 대기) ${deviceTag}`);
    return { status: 'OK' };
  } catch (e) {
    await logActionToNotion(cleanBiz, name, `승인 요청 실패 ${deviceTag}: ` + e);
    return { status: 'ERROR', msg: e.message };
  }
}

// 비밀번호 설정
async function setAuthPassword(bizNo, newPw, isMobile) {
  const cleanBiz = String(bizNo).replace(/[^\d]/g, '');
  const user = await queryAuthDb_(cleanBiz);
  ...
  if (!user) {
    await logActionToNotion(cleanBiz, bizName, `비밀번호 설정 실패: 사용자 없음 ${deviceTag}`);
  ...
  if (pastPwList.includes(targetPw) || pastPwList.includes(base64Pw) || pastPwList.includes(hashedPw)) {
    await logActionToNotion(cleanBiz, bizName, `비밀번호 재사용 시도됨 ${deviceTag}`);
    return { status: 'USED_PW' };
  }

  // 통과 시 진행
  try {
    await updateAuthPage_(user.pageId, {
      '현재PW': { rich_text: [{ text: { content: hashedPw } }] }
    });
    await logActionToNotion(cleanBiz, bizName, `비밀번호 설정 완료 ${deviceTag}`);
    return { status: 'OK', custName: bizName, repName: repName, tutPc: user.tutPc, tutMo: user.tutMo };
  } catch (e) {
    await logActionToNotion(cleanBiz, bizName, `비밀번호 설정 시스템 에러 ${deviceTag}: ` + e);
    return { status: 'ERROR', msg: e.message };
  }
}
...
// 로그인시도
async function tryLogin(bizNo, inputPw, isMobile) {
  const cleanBiz = String(bizNo).replace(/[^\d]/g, '');
  const user = await queryAuthDb_(cleanBiz);
  ...
  if (!user) {
    await logActionToNotion(cleanBiz, '알수없음', `로그인 실패: 사용자 없음 ${deviceTag}`);
    return { status: 'ERROR', msg: '사용자 없음' };
  }
  ...
  if (isMatch) {
    ...
    if (Object.keys(propsToUpdate).length > 0) {
      await updateAuthPage_(user.pageId, propsToUpdate);
    }

    const config = await initDcConfigFromNotion(cleanBiz);
    await logActionToNotion(cleanBiz, bizName, `로그인 성공 / 설정 로드됨 ${deviceTag}`);
    ...
    if (newCount >= 3) {
      ...
      status = 'LOCKED';
      await logActionToNotion(cleanBiz, bizName, `비밀번호 3회 오류로 계정 제한 ${deviceTag}`);
    } else {
      await logActionToNotion(cleanBiz, bizName, `로그인 실패 (비밀번호 불일치 ${newCount}회) ${deviceTag}`);
    }
    await updateAuthPage_(user.pageId, props);
    return { status: status, count: newCount };
  }
}

// 인증 DB 데이터 조회
async function queryAuthDb_(bizNo) {
...
// 주문서 사용기한 계산 로직
async function getAccessExpiration(bizNo) {
  const cleanBiz = String(bizNo).replace(/[^\d]/g, '');
  const user = await queryAuthDb_(cleanBiz);
  if (!user) return { status: 'ERROR' };

  const NOTION_TOKEN_LOG = (process.env.NOTION_TOKEN_LOG || process.env.NOTION_TOKEN_SNAPSHOT || '__MISSING_NOTION_TOKEN_LOG__');
  const NOTION_DB_ID_LOG = '2eda1006d65880d696b3da4a8d281ea2';
  const NOTION_TOKEN_SHIPPING = (process.env.NOTION_TOKEN_SHIPPING || process.env.NOTION_TOKEN_SEND || '__MISSING_NOTION_TOKEN_SHIPPING__');
  const NOTION_DB_ID_SHIPPING = '2f8a1006d658803face6fdfe2b175780';

  // 로그 및 출고 내역에서 최신 시점 조회
  const getLatestTime = async (dbId, isLog) => {
...
// 튜토리얼저장
async function saveTutorialState(bizNo, isMobile) {
  try {
    const cleanBiz = String(bizNo).replace(/[^\d]/g, '');
    const user = await queryAuthDb_(cleanBiz);
...
// 인증 DB 데이터추가
async function createAuthRow_(bizNo, name) {
...
// 인증 DB 데이터 업데이트
async function updateAuthPage_(pageId, props) {
...
/* 주문내역가져오기 */
async function getOrderHistory(bizNo, dateType, startDate, endDate) {
...
// 주문 저장
async function saveOrderToNotion(common, items, ioNo) {
  const url = 'https://api.notion.com/v1/pages';
  const headers = {
    'Authorization': 'Bearer ' + NOTION_TOKEN_ORDER,
    'Notion-Version': '2022-06-28',
...
// 동작 로그 노션 기록
async function logActionToNotion(bizCode, bizName, message) {
  const NOTION_TOKEN_LOG = (process.env.NOTION_TOKEN_LOG || process.env.NOTION_TOKEN_SNAPSHOT || '__MISSING_NOTION_TOKEN_LOG__');
  const NOTION_DB_ID_LOG = '2eda1006d65880d696b3da4a8d281ea2';
...
// 프론트엔드 동작 로그 중계
async function logFrontEvent(bizNo, action, detail, isMobile) {
  if (!bizNo) return;
  ...
  // 기존 노션 기록 함수 호출
  await logActionToNotion(cleanBiz, name, `[${action}] ${detail} ${deviceTag}`);
}
// ─── legacy Code.js 본문 끝 ────────────────────────────────────────────────
