...
/** ApprovalLineAuthorizeClient 운영 생성자 DI 가드 테스트. */
class ApprovalLineAuthorizeClientDiGuardTest {

    private final ApplicationContextRunner contextRunner = new ApplicationContextRunner()
            .withUserConfiguration(ClientDiGuardConfig.class);

    @Test
    void approvalLineAuthorizeClient_operationalConstructor_isAutowired() {
        contextRunner.run(context -> assertThat(context)
                .hasNotFailed()
                .hasSingleBean(ApprovalLineAuthorizeClient.class));
    }

    @Configuration(proxyBeanMethods = false)
    @Import(ApprovalLineAuthorizeClient.class)
