...
/**
 * X-Internal-Token 헤더 인증 — Phase 10 W10-4 (PR #99) slip-service 신규.
 *
 * <p>{@code /internal/**} prefix 한정 (arologis-service / partner-service 와 일관). 그 외 경로
 * (delivery / public / slips 일반 admin 등) 는 즉시 통과 → downstream {@link HeaderAuthenticationFilter}
 * 가 X-User-* 헤더로 일반 사용자 흐름 처리.
 *
 * <p>토큰 일치 → ROLE_MASTER 권한으로 통과. 토큰 미제시 → no-op (downstream 처리, 미인증 상태로
 * Spring Security 가 401/403). 토큰 불일치 → 401 즉시 응답 (filter chain 단절).
 */
