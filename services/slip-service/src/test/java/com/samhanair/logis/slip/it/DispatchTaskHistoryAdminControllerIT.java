...
class DispatchTaskHistoryAdminControllerIT extends AbstractPostgresIT {
    ...
    void list_filters_status_date_and_paginates_summary() throws Exception {
        ...
        mvc.perform(get("/admin/dispatch-tasks")
                ...
                .andExpect(jsonPath("$.content[0].taskCode").value(first.task().getTaskCode()))
                .andExpect(jsonPath("$.content[0].status").value("DISPATCHED"))
                .andExpect(jsonPath("$.content[0].vehicleGroupCount").value(1))
                .andExpect(jsonPath("$.content[0].slipCount").value(1))
                .andExpect(jsonPath("$.content[0].partnerNames").value("거래처A"))
                .andExpect(jsonPath("$.content[0].driverCount").value(1))
    ...
    void detail_populates_vehicle_groups_slips_and_matched_drivers() throws Exception {
        ...
        mvc.perform(get("/admin/dispatch-tasks/{taskId}", seeded.task().getId())
                        ...
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(seeded.task().getId().toString()))
                .andExpect(jsonPath("$.taskCode").value(seeded.task().getTaskCode()))
                .andExpect(jsonPath("$.dispatchDate").value(BASE_DATE.toString()))
                .andExpect(jsonPath("$.status").value("DISPATCHED"))
                .andExpect(jsonPath("$.arologisDispatchId").value(seeded.task().getArologisDispatchId().toString()))
                .andExpect(jsonPath("$.vehicleGroups[0].id").value(seeded.group().getId().toString()))
                .andExpect(jsonPath("$.vehicleGroups[0].vehicleType").value("TONNAGE_1"))
                .andExpect(jsonPath("$.vehicleGroups[0].sequence").value(1))
                .andExpect(jsonPath("$.vehicleGroups[0].slips[0].id").value(seeded.mapping().getId().toString()))
                .andExpect(jsonPath("$.vehicleGroups[0].slips[0].slipId").value(seeded.slip().getId().toString()))
                .andExpect(jsonPath("$.vehicleGroups[0].slips[0].sequence").value(1))
                .andExpect(jsonPath("$.vehicleGroups[0].slips[0].slip.slipNo").value(seeded.slip().getSlipNo()))
                .andExpect(jsonPath("$.vehicleGroups[0].slips[0].slip.partnerCode").value("P-HIST-004"))
                .andExpect(jsonPath("$.vehicleGroups[0].slips[0].slip.partnerName").value("상세거래처"))
                .andExpect(jsonPath("$.vehicleGroups[0].slips[0].slip.deliveryAddress").value("서울시 강남구 4"))
                .andExpect(jsonPath("$.vehicleGroups[0].slips[0].slip.recipientPhone").value("010-0000-0004"))
                .andExpect(jsonPath("$.vehicleGroups[0].slips[0].slip.dispatchStatus").value("DISPATCHED"))
                .andExpect(jsonPath("$.matchedDrivers[0].vehicleGroupSequence").value(1))
                .andExpect(jsonPath("$.matchedDrivers[0].driverCode").value("DRV-004"))
                .andExpect(jsonPath("$.matchedDrivers[0].driverName").value("기사4"))
                .andExpect(jsonPath("$.matchedDrivers[0].driverPhoneNumber").value("010-9999-0004"))
                .andExpect(jsonPath("$.matchedDrivers[0].driverSource").value("AROLOGIS"));
    }
