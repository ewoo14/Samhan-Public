...
/**
 * Gateway 가 주입한 {@code X-User-Id} + {@code X-User-Role} 헤더를 신뢰하여
 * 사전 인증된 {@link UsernamePasswordAuthenticationToken} 을 SecurityContext 에 적재한다.
 *
 * <p>이렇게 하면 controller 의 {@code @PreAuthorize("hasRole('MASTER')")} 등이 정상 동작.
 * gateway 우회 직접 호출 (개발자 로컬 등) 시에는 헤더 미존재로 인증 비적재 → 401.
 *
 * <p>본 filter 는 {@link InternalTokenFilter} 가 internal token 미적재로 통과시킨 경우의
 * downstream 처리만 담당.
 */
