...
/**
 * MinIO (S3 호환) 백엔드 구현. {@code app.partner.minio.enabled=true} 일 때만 활성화.
 *
 * <p>비활성 환경 (CI / 단위 테스트) 에서는 {@link NoopAttachmentStorage} fallback 이 주입.
 *
 * <p>endpoint = {@code app.partner.minio.endpoint} (default {@code http://localhost:9000})
 * — docker-compose 의 {@code samhan-minio} container.
 */
@Component
@ConditionalOnProperty(value = "app.partner.minio.enabled", havingValue = "true")
public class MinioAttachmentStorage implements AttachmentStorage {
    ...
    void init() {
        ...
        try {
            ...
            log.warn("MinIO bucket init failed (storage 호출 시 재시도): {}", ex.getMessage());
        }
    }
