...
public interface ApprovalLineConfigRepository extends JpaRepository<ApprovalLineConfig, UUID> {
    ...
    /** 전표 종류별 활성 역할 중 가장 뒤 sequence 를 조회한다(@SQLRestriction 으로 soft-delete 제외). */
    Optional<ApprovalLineConfig> findFirstByDocumentTypeOrderBySequenceDesc(String documentType);
