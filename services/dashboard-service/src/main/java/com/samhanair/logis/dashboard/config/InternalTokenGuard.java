 ...
 * 가동되는 사고를 차단한다. partner-service / groupware-service / notification-service 의
 * 동일 가드 패턴.
 */
@Slf4j
@Component
...
public class InternalTokenGuard {
    ...
    /**
     * Spring 컨텍스트 초기화 시점 검증 — prod + dev 기본값 → 부팅 거부, 비프로덕션 + dev
     * 기본값 → 경고 로깅.
     *
     * @throws IllegalStateException prod 프로파일 + dev 기본 토큰
     */
    @PostConstruct
    void verify() {
        ...
        if (isProd && isDevDefault) {
            throw new IllegalStateException(
                    "SAMHAN_INTERNAL_TOKEN(=app.security.internal.token) 가 dev 기본값('"
                            + DEV_DEFAULT
                            + "') 인 채로 prod 프로파일이 활성화되었습니다. "
                            + "환경변수 SAMHAN_INTERNAL_TOKEN 을 안전한 값으로 설정 후 재기동하세요.");
        }
        if (isDevDefault) {
            log.warn("[보안] app.security.internal.token 가 dev 기본값입니다. "
                    + "비프로덕션이면 무시. 배포 전 SAMHAN_INTERNAL_TOKEN 환경변수로 교체 필요.");
        }
    }
}
