...
**잔여**: 실 부서명·간이 계정과목 seed(개발책임자 제공 대기), Codex 회복(Jun 11) 후 추가 크로스 실서버 테스트. ⚠️ 본 세션 QA 가 실 auth_db/arologis_db 를 main HEAD(auth V52/arologis V15)까지 전진 적용 → 로컬 dev 스택(:8081/:8097)은 stale 코드, 다음 `docker compose up --build` 재빌드 권장.
