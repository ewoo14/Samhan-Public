...
진행: **에픽 pre-cutover 스코프 완결**(슬1·시드정합·슬2·슬4·슬6 6a+6b 전부 머지, 2026-06-20). 잔여는 Phase11 cutover 시점(6c silo 폐기·D3 물리 DROP·D4) — **cutover 마무리 runbook 작성 완료**(`docs/migration/2026-06-20-ecount-native-fold-cutover-runbook.md`, Step A 주문 이식 실행→B 6c silo 폐기→C D3 DROP→D D4, 검증 게이트). 메인 ECOUNT-CUTOVER-GUIDE 교차참조. 6c/D3 코드는 bit-rot 회피 위해 cutover 시점 작성(runbook 에 파일/변경 명세). 슬6 dev 실데이터 QA는 cutover 시(품목 import cross-DB 갭으로 dev 변환 불가→IT 실 Postgres 검증). **다음=개발책임자 지정 다음 우선작업**(품목/견적 에픽 잔여·외부연동 등). 🪤 슬6 듀얼리뷰가 mocked-IT false-green 뒤 P1 보안 fail-open(위조 X-User-Id export)+categoryKey 계약+price_vat/converted 의미버그 단독 적발→라이브 검증으로 폐쇄.

🪤 **cutover dry-run 단독 적발·수정(#529, issue #528, 2026-06-20)**: 개발책임자 지정 dry-run(품목 import 파이프라인 dev 로드) 중 **MIG-8 주문 변환 cross-DB 버그** 발견 — `Mig8OrderTransformService` 가 `accounting_db.staging.ecount_item_alias`(미존재, product-service V7 가 product_db 에만 생성)를 직접 쿼리 → 상시 실패 = accounting.orders 상시 0 = silo 빈화면의 진짜 원인. **변환이 한 번도 작동한 적 없는 cutover 블로커.** → product-service `POST /products/internal/resolve-ecount-aliases`(소유 테이블 배치 해석) 경유 수정. 품목 import = CSV(3종 dev raw 존재, eCount API키 불요). dev 라이브 end-to-end 는 시드 product `ux_products_model_name_active` 충돌(dev artifact, cutover fresh DB 무관)로 정지→IT 검증. **교훈: 예행연습이 실 운영 장애를 사전 발굴.** Opus 교차가 본 fix 라운드 커밋 누락(미push)도 단독 적발→보강. cross-service 데이터 변환은 소유 서비스 경유(service-per-DB).
