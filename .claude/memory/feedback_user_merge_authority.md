...
**🆕 2026-06-03 추가 위임 — "머지도 PM 판단"**: 개발책임자 명시 — **P1 잔존·UNSTABLE 등 비-0결함 상황도 PM 이 머지 여부를 자율 판단**(이전엔 결함 시 사용자 대기였으나 이를 PM 위임). 판단 기준: happy-path 무결(CI green + Docker 실 QA PASS) + 잔존 결함이 설계 수준/발생조건 제한적/후속 슬라이스 분리 가능하면 PM 머지 + 후속 슬라이스 기록. 단 데이터 손실·보안·운영중단급 결함은 여전히 보류. (S4 #348 회수연동: Codex P1 2건[동시성·보상 인프라, S3 공통]을 happy-path 무결로 PM 판단 머지 + "시리얼 동시성·보상 강화" 후속 분리.)

**예외 (사용자 결정 대기)** — 위 위임 후에도 보류:
- CI fail (green 아닌 실패) → fix 후 재시도, fix 불가 시 보고
- 데이터 손실/보안/운영중단급 P0 → 머지 금지, 보고
- 메모리 가드 위반 (한국어 X / 직접 set / UUID 노출 등) → PM 추가 commit 자가 fix 후 재시도
