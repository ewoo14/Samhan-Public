...
**How to apply:** 색 토큰화 fix는 (1) `design-system/src/tokens/tokens.css`에서 **토큰 실제값 확인**, (2) fallback과 다르면 렌더 색이 바뀜을 인지, (3) WCAG 대비 재계산으로 AA(일반텍스트 4.5:1) 확인 후에만 "값 불변/개선" 주장. 실제값==fallback인 경우만 안전(예 `--color-neutral-600`=#4D5562). 안전 최소위험 = raw hex 유지 or 실제 AA 통과 토큰 선택. [[feedback_review_5agent_no_shortcut_strict]] 재검이 회귀 포착한 사례.
