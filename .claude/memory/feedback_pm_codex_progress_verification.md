...
**2026-06-07 추가 확정**: Codex 디스패치는 **codex exec 백그라운드 방식이 표준** (개발책임자 "권장" 선택). MCP 동기 호출은 침묵 구간 중 사용자 메시지(10분 주기 보고 포함)가 호출을 즉시 취소시키는 패턴이 3회 반복되어 폐기. `</dev/null` stdin 리다이렉트 + 로그 파일 모니터링 + 3분 무로그 = hang 판정.
