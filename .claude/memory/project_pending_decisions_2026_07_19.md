...
3. **전표 거래처 필수화 = 단계적**. FE 즉시 필수화 + 기존 null 전표(2271중 1942·86%) 조사·보정 후 BE `slips.partner_id` NOT NULL. #823·#825 슬1/2 위에 얹힘.
...
5. **#848 documentType 오버플로 = 컬럼 40→70 확장**(groupware Flyway V11). `ApprovalLineBase.document_type` 콘크리트 엔티티는 groupware `ApprovalLine` 1개뿐 → blast radius=groupware_db 한정. 현 code max 14자(오버플로 0건·잠재 제약). code≤30 상한(입력템플릿 부작용)·접두사 제거(계약 파급) 비채택.
