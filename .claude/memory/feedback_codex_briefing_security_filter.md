...
## 🚨 2차 실측 — **한 번 바꾼 걸로는 부족했다**

같은 PR 의 다음 판정 라운드가 **또 걸렸다**. 1차에서 이미 순화한 브리핑이었는데도다.

```text
1차 걸림 → 순화 → 통과 → (다음 라운드) 같은 순화 수준으로 발주 → 🚨 또 걸림
```

순화본에 **아직 남아 있던 표현**:

```text
"**게이트웨이를 통과시키는** 데 필요한 환경변수"   ← 통과시키다 = 우회로 읽힌다
"저장 기능이 MANAGER **역할**로도 동작"
"**역할 기반** 기능은 해당 **마이그레이션이 반영된** 격리 DB 에서 확인하라"
"**자격 값**은 resolveQaCredential() 로 얻어라"
⟹ 역할·자격·통과가 한 브리핑에 다시 모였다
```
...
| 🚫 아직 걸리는 표현 | ✅ 더 안전한 표현 |
|---|---|
| `게이트웨이를 통과시키는 데 필요한 환경변수` | *"환경 설정은 이전 라운드 보고서의 실행 절차를 그대로 따르라"* |
| `MANAGER 역할로도 동작` | *"관리 화면에서도 저장이 된다"* |
| `역할 기반 기능` / `마이그레이션이 반영된 DB` | *"해당 기능이 켜진 환경에서"* |
| `자격 값은 resolveQaCredential() 로` | *"로그인은 기존 QA 헬퍼를 쓰라"* |

🔑 **역할·자격·권한·통과·주입 중 두 개 이상이 한 브리핑에 있으면 걸린다고 보라.**
환경 구성은 **문장으로 설명하지 말고 "이전 보고서 절차를 따르라" 로 넘겨라.**
