...
**진행 순서**: ~~F1.5✅(#504)~~ → ~~F3✅(#505)~~ → **F4 (다음 — ⚠️ 견적 출력 변경, 개발책임자 검토/go 대기)**: 옵션 토글 자동매칭 B·isDefault 우선·setModel 그룹, **F1.5 panelType/remoteType + BundleExpander.pickPanel attribute 기반 전환**(현 런타임 문자열 매칭→DB attribute); classifyRemoteType 가 variant 도 읽도록 보강 필요 → **F5**(estimate-app 설정 기반 계산 전환·golden parity 회귀). ※ F4=BundleExpander 매칭 동작 변경=견적 출력 영향 → 야간→오전 경계 + 착수 전 확인 제안으로 개발책임자 go 대기(2026-06-19). go 시 golden parity 회귀 필수.
