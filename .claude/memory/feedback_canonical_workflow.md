...
## 병렬도 — 🚨 2026-07-23 개발책임자 재지시가 현행 정본
> *"가급적이면 트랙 3개 모두 병렬로 **꼭** 진행 요청"*

**3트랙 병렬이 기본 운영 상태이고, 트랙이 비면 채운다.** 한 트랙이 스테이지를 끝내면 회수·검증·게시 직후 **곧바로** 다음 스테이지를 디스패치한다. ⚠️ **07-22 의 "마감 국면에서는 병렬 축소" 는 폐기** — 아래 07-22 실패는 병렬 자체가 아니라 **트랙 간 자원 분리를 안 한 것**이 원인이었고, 포트·DB·워크트리를 갈라 배정하면 실측상 3트랙이 간섭 없이 돈다(2026-07-23 실증). 분리 항목 상세는 [[feedback_throughput_parallel_scope_freeze_batch]].

07-22 에 관측된 병렬 부작용은 **여전히 유효한 경고**이니 병렬을 유지하면서 이것들을 따로 막아라: PM 게시 누락(LOW 20건 압축) · PM 이 요구한 가드의 `tests=0` 구멍 · 스크린샷 클로버 3회 · 격리본 오적용. 방어책 = **실행=게시 1:1 엄수**(PR 이 상태 저장소) · 트랙별 QA 산출물 디렉토리 분리 · 스크린샷 원복은 경로 한정.

## 리뷰 라운드의 에이전트 구성 — **루트 SOL 이 도달성+라이브QA 를 겸하는 것이 옳다** (2026-07-23)
`mcp__codex__codex` 로 SOL 라운드를 돌리면 SOL 이 알아서 서브에이전트를 뿌린다. #907 라운드 실측 = rollout **5스레드 = 4 전문 에이전트 + 루트 1**:
```
fe_state 681KB · be_contract 571KB · security_auth 886KB · regression_shared 554KB
루트 2,737KB (종합 + 라이브QA 직접 수행 + 도달성/사용자 경로 차원)
```
PM 이 5차원을 주문했는데 "도달성/사용자 경로"만 전용 에이전트가 안 생기고 루트가 흡수한 것을 보고 **PM 이 "도달성 전용 에이전트를 두자"고 제안했으나, 개발책임자가 "루트 SOL 을 놓은 것은 맞는 판단"이라고 정정**했다.

🔑 **왜 루트가 맞나**: **도달성 = "실 사용자 경로로 재현되는가" = 라이브QA 그 자체**다. 루트가 라이브QA(실서버 기동·throwaway 생성·스크린샷·양성 대조)를 수행하므로 도달성 판정은 루트에 있어야 한다. 전용 에이전트로 떼면 ①라이브 서버 없이 도달성을 논하는 리뷰어가 되거나 ②라이브QA 가 중복 수행된다. 실제로 이 구성에서 루트가 R-1(후보/가드 불일치)·R-2(상태 잔존)·R-3(캐시 stale)을 **실행 재현**해 잡았다.
⟹ **전문 차원(BE 계약·보안·FE 상태·회귀)은 서브에이전트, 도달성+종합+라이브QA 는 루트.** 프롬프트에 차원을 나열하되 도달성을 서브에이전트로 강제하지 말 것.
⟹ 다만 회수 후 rollout 의 `/root/<name>` 으로 **실제 에이전트 구성을 확인하는 습관은 유지**하라 — 주문한 차원 수와 실제 생긴 에이전트 수는 다를 수 있고, 전문 차원이 통째로 누락됐다면 그건 진짜 문제다.
※ 구현·fix 스테이지(LUNA)는 **단일 에이전트가 정상** — 3트랙 전부 서브에이전트 0 으로 확인됐고, 캐논이 다중 에이전트를 요구하는 것은 **리뷰 스테이지뿐**이다(리뷰어/구현자 분리).
...
9. 🚨 **라이브QA = 실서버 실제 실행. 정적 게이트로 대체 금지 (2026-07-21 개발책임자 지시)**
   ...
   ### 🚨 2026-07-22 개발책임자 지시 — **라이브QA 는 PM 이 직접 수행한다**
   > *"아무래도 병렬 트랙으로 진행하니 QA 품질이 어긋날 수 있어. 그래서 라이브 QA는 따로 서브에이전트를 돌리지 말고 **PM이 직접 검증**하는 방식을 사용하면 좋겠어. 그래야 **전체 흐름을 알고 있는 PM이 전체적인 관점에서 더 넓게 확인**이 가능하니까. 다만, PM이 세부적인 QA를 모두 진행하기 어려운 경우 **자율판단 하에 QA 에이전트를 따로 두고 함께 검증도 가능**은 해."*

   - **원칙 = PM 직접 수행.** 🚫 라이브QA 를 서브에이전트에 **위임(대체)하지 않는다.** 적대검증 라운드의 다른 차원(FE/BE/Design/DevOps/QA 리뷰)은 종전대로 서브에이전트로 디스패치하되, **라이브QA 만은 PM 이 손으로 돌린다.**
   - **취지**: ①병렬 트랙에서 각 서브에이전트는 자기 트랙만 보므로 **QA 품질이 트랙마다 어긋난다** ②PM 은 전 트랙의 발견·결정·이력을 들고 있어 **트랙 간 상호작용과 회귀를 넓게** 볼 수 있다(예: 공유 서비스·공유 DB·공유 컴포넌트).
   - **예외(PM 자율)**: 세부 QA 를 PM 이 전부 수행하기 어려우면 **QA 에이전트를 두고 병행**할 수 있다. 단 **대체가 아니라 보조**이며, PM 이 직접 확인한 부분과 에이전트가 확인한 부분을 **구분해 게시**한다.
   - ⚠️ **PM 이 직접 한다는 것은 "돌렸다" 가 아니라 "봤다" 를 뜻한다** — 배포 증명(이미지·jar 심볼·flyway·`ddl-auto=validate`), 실 GUI 스크린샷, 실패 시 원문을 PM 이 직접 확인한다([[feedback_pm_verify_what_measurement_proves]]).

   ### 🚨 2026-07-23 개발책임자 지시 — **CODEX SOL 라운드도 SOL 이 직접 라이브QA 를 수행한다**
   > *"코덱스 리뷰라운드에서는 SOL이 직접 라이브 QA를 진행하는지 문의.(실제로 그래야함)"*

   - **실측 실태(2026-07-23 확인) — 라운드마다 달랐다:**
     - **#908 SOL 라운드 = 수행함** ✅ — 실 브라우저 rect 값을 제출했다(`좌표 레이어 90.7px`(=24mm@96dpi) · `맺음말 겹침 19.42px` · `맺음말 중앙 hit target = text-1`). 소수점과 `elementFromPoint` 결과는 실행 없이는 나오지 않는다.
     - **#907 SOL 라운드 = 수행 안 함** ❌ — `gradlew --rerun-tasks` · vitest · `git diff --check` **테스트 실행만** 하고, SOL 스스로 *"파일 수정·공유 DB 쓰기 없음 — 리뷰어 제약 준수 확인"* 이라고 적었다. **PM 이 건 제약이 라이브QA 를 원천 차단했다.**
   - ⟹ **SOL 라운드의 라이브QA 는 필수다.** PM 라이브QA 와 **별개의 독립 실행**이며 서로 대체하지 않는다. 두 검증 스테이지가 이종(異種)인 가치는 **같은 것을 다른 눈으로 "실행"해 볼 때** 나온다 — 정적 리뷰만 두 번 하는 것은 이종 검증이 아니다.
   - **PM 디스패치 의무** — SOL 리뷰 프롬프트에 반드시 명시한다:
     1. 실서버 기동 상태·접속 정보(게이트웨이 URL·계정)와 **FE 하네스 기동 방법**
     2. **라이브QA 수행 지시** + 실행 원문·스크린샷 **제출 의무**
     3. **쓰기 허용 범위** — throwaway 전용, 공유 실데이터(실 템플릿·마스터·설정) write 금지, 종료 시 정리 의무([[feedback_qa_live_shared_data_readonly]])
   - 🚫 **"파일 수정·DB 쓰기 금지" 를 통째로 걸어 라이브QA 를 막지 않는다.** 금지 대상은 **git 조작**과 **공유 실데이터 오염**이지 **라이브 실행이 아니다.** 이 구분을 프롬프트에서 흐리면 SOL 은 정적 분석으로 후퇴한다(#907 실측).
   - 🚫 SOL 이 라이브QA 를 못 했으면 **그 라운드는 미완**이다. 정적 리뷰만으로 라운드 종료를 선언하지 않는다.
