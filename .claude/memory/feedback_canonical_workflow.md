...
## 🚨 2026-07-22 (2차) 개발책임자 정정 — **PM 재량 명문화 · 후속 분리 지양**

> *"후속 분리는 가급적 지양 부탁."*
> *"fix 는 새 결함 유발하는 경우로 인해 도달 가능 머지 방식으로 변경했으니 큰 문제가 안된다면 PM 판단하에 머지 가능."*
> *"가급적 사용자 관점에서 오류가 발생하지 않거나, 보안 이슈가 아니라면 PM 이 판단하에 머지 가능."*

**⟹ 게이트를 실질 기준으로 좁힌다.**

| 구분 | 처리 |
|---|---|
| **사용자 관점에서 실제 오류가 발생**(기능 실패·데이터 손실/오류·잘못된 결과·도달 불가) | 🚫 **머지 불가** |
| **보안·권한 이슈** | 🚫 **머지 불가** |
| 그 외 — UX 개선 여지 · 디자인 일관성 · 표기/문구 · 편의기능 미비 · 검증 품질 | ✅ **PM 판단하에 머지 가능** |

🔑 **핵심 취지**: 도달성 축으로 바꾼 이유 자체가 **"fix 가 새 검증장치를 낳고 그게 다음 라운드 감사 대상이 되는 비종료 루프"** 를 끊기 위함이었다. 그런데 `도달 가능` 을 **문자 그대로 "사용자가 마주치는 모든 것"** 으로 넓게 읽으면 같은 루프가 재발한다. **판정 질문은 "사용자가 이걸 겪는가" 가 아니라 "사용자가 이것 때문에 오류를 겪는가 / 보안이 뚫리는가" 다.**

🚫 **후속 분리 지양** — 게이트가 아닌 항목도 **가능하면 현 PR 에서 처리**한다([[feedback_backlog_burndown_issue_bar]] 와 동일 방향). 다만 **그것이 머지를 막지는 않는다**. 즉 *"고칠 수 있으면 지금 고치되, 못 고쳤다고 머지를 붙잡지 마라."*

⚠️ **PM 이 남용하지 말 것**: 재량은 *"오류도 보안도 아님"* 이 **논증될 때만** 성립한다. 판정 근거(어떤 사용자 경로에서 무엇이 일어나며 왜 오류가 아닌지)를 **PR 에 명시**하고 머지한다. 애매하면 오류 쪽으로 본다.
