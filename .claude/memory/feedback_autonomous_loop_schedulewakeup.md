...
🚨 2026-06-24 개발책임자 지시(+ 같은 날 보강). **워크플로우 진행 시 매 단계마다 `ScheduleWakeup`으로 재자각하며 진행한다 — 사용자 부재(취침/외출)뿐 아니라 사용자 활성 중에도 동일 적용(개발책임자 "재개 후에도 매 단계 사용" 명시).** 즉 연속 mega-턴으로 몰지 말고, 단계(또는 1~2단계 묶음) 완료 시마다 다음 단계를 ScheduleWakeup 으로 예약·재자각하며 chunk 한다.
