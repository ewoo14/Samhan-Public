...
- [🚨 "지방"은 **태그**다 — 배송주소 접두 아님 (2026-08-01 개발책임자 정정)](feedback_region_is_a_tag_not_address_prefix.md) — `slips.delivery_tag='REGION'`(DAY·STACK·RETURN_RENTAL 계열). ❌`지방/` 접두 파싱=이카운트 방식 ❌17개 시도 문자열=`RegionalService` 현행인데 **틀림**. 🔑레거시 표식은 "어떻게 표시했나"라 계승 대상 아님. ⚠️REGION 12건뿐 — 태그 기반 전환 전 실제 건수 대조 필요
