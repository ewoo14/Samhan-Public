...
- [🚨 표준 워크플로우 유일 진실원 (07-15 개편 · 07-20 기획검수 폐지 · 🚨07-21 현행 정본 리뷰=OPUS 4.8·fix=SONNET5)](feedback_canonical_workflow.md) — **OPUS 4.8 기획**(조기 PR 개설+기획 리뷰 게시) → **CODEX LUNA 5.6 구현**(게시) → (**OPUS 4.8 5+agent 적대리뷰·라이브QA** + **SONNET5 fix**+검증+게시 → **CODEX SOL 5.6 5+agent 리뷰**+**LUNA fix**+게시) **전 심각도 0수렴까지 반복** → PM 종합(게시) → CI green → PM 머지. **🚨🚨2026-07-21 21:40 현행 정본 — 수렴 기준 = 전 심각도(BLOCKING/HIGH/MED/LOW) 잔여 0**("기존 MED, LOW 등의 결함도 모두 FIX하고 넘어가는것으로 하자"). ⚠️**같은 날 앞선 "BLOCKING/HIGH 0 게이트·MED/LOW 배치이슈" 결정은 폐기**(구 문구를 현행으로 읽지 말 것). 그 문제의식(fix 가 새 결함→라운드 무한증식)은 **범위 축소가 아니라 fix 품질로 해결** = **RED-first + 라이브QA 실서버 실행**. 범위 밖(타 서비스·타 슬라이스)만 분리 가능. 📌실측은 유효: fix 부피가 다음 라운드 발견량을 키움(#854 R4 신규1,400줄→R5 HIGH 거의 전부 그 안·#864 R3 HIGH/MED 5중 4가 R2 fix 코드·#865 R2 MED 4중 3이 R1 fix 도입) → 전건 fix 하되 RED-first 로 표면 확정해 부피·오사격 축소. test.skip/false-green/미실행/캐시인용은 심각도 무관 통과 아님·강도·5차원·라이브QA·게시1:1·라운드 상한없음 불변. **🚨PM 은 fix 지시에서 불변식만 말하고 구현 수단 지시 금지**(수단 지시하면 그 결함까지 PM 이 떠안음 — #865 M-1 COALESCE 감사위조·#865 ACTIVE-0 수용처분·#864 HIGH-1 FE만 보고 판정, 3건 전부 PM 발). **🚨2026-07-21 지시: 1차 적대검증 리뷰=FABLE5→OPUS 4.8 재전환**(FABLE5 토큰 과다 — 6차원 라운드 1회 약 1.0~1.2M 실측), **fix 는 SONNET5 그대로 유지** → 리뷰어(OPUS)/구현자(SONNET5) 분리 존속. **OPUS 4.8=기획+1차 적대검증 리뷰+PM 오케스트레이션**(라운드 fix 직접 수행만 금지). 같은 사유 3번째 전환(07-16 폐지→07-20 복원→07-21 재폐지). 🚫엄수·단축금지·순차(병렬금지)·**모든 단계 리뷰 게시(실행=게시 1:1)**·**라이브QA 스샷 다수 필수**·fix=라운드모델·false-positive 의심도 재수렴+PM종합 후·모델 대체 금지(부재 시 개발책임자 선확인)·9-게이트·근본원인 자각(속도<충실도)·매단계 ScheduleWakeup. 모델 ID 실측: `gpt-5.6-sol`/`gpt-5.6-luna`. **🚨2026-07-20 기획검수(구 CODEX SOL spec검수 2단계) 폐지=적대리뷰 중복·spec점검은 OPUS 기획 흡수**. ※**과거 이력(현행 아님)**: 07-16 FABLE5 폐지→OPUS 겸임 → 07-20 2차 FABLE5 복원(`model:"fable"`) → **07-21 재폐지(위 현행 정본)**. 07-20 2차 문구를 "현행"으로 읽지 말 것. **구 워크플로우 전부 폐기**
