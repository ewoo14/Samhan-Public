...
- [🚨 표준 워크플로우 유일 진실원 (🚨🚨07-22 머지게이트=도달성 축 · 07-21 리뷰=OPUS 4.8·fix=SONNET5)](feedback_canonical_workflow.md) — **OPUS 4.8 기획**→**CODEX LUNA 5.6 구현**→(**OPUS 4.8 적대리뷰·라이브QA**+**SONNET5 fix**→**CODEX SOL 5.6 리뷰**+**LUNA fix**) 수렴까지 반복 → PM 종합 → CI green → PM 머지. **🚨🚨2026-07-22 현행 정본 — 머지 게이트 = ①실 사용자 경로로 재현 가능한 결함 0(심각도 무관) ②CI green(exact SHA) ③라이브QA(실서버 실행)**. ⚠️**07-21 "전 심각도 0" 은 폐기** — 종료 조건이 아님이 실측됐다(밤새 3트랙 머지 0건, BLOCKING/HIGH/LOW 는 수렴하는데 MED 만 7 근처 평평, 그 MED 의 과반이 "fix 가 추가한 방어선이 가짜" 류 **검증 품질**). 🔑비종료 이유=**fix 하면 검증 장치가 늘고 그게 다음 라운드 감사 대상이 됨**(fix-유발률 76~100% 실측, RED-first 도 못 막음 — 리뷰어가 RED-first 테스트를 다시 감사). **검증 품질(테스트 약함·문서 과장·가드 구멍·mock 미비·직접SQL전용)은 게이트 아님 → 슬라이스당 1이슈 이월+분기 chore 배치**. 🚫"MED/LOW 무시" 아님 — **도달 가능한 MED/LOW 는 여전히 게이트**, 축이 심각도→도달성으로 바뀐 것. 리뷰어는 발견을 `도달가능`/`검증품질` 로 **분류 의무**, 마감 라운드는 **도달성 단일 질문으로 좁힘**(전면 감사 반복 시 재발). 유효 존속=RED-first·라이브QA 실서버·**뮤테이션 RED 의무(07-22 신설)**·PM 중재 3단·"RED 불가=고치지 않는다"·게시1:1·모델 대체 금지·PM 은 불변식만. **병렬도=마감 집중 후 재개**(3병렬이 게시 누락·가드 구멍·스샷 클로버 3회를 만듦). 모델 ID: `gpt-5.6-sol`/`gpt-5.6-luna`
