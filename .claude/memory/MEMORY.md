...
- [🚨🚨 **PM 이 라운드마다 점검하고 fix 지시를 구체화한다 — 단 "무엇"까지만** (2026-08-05 개발책임자)](feedback_pm_gives_coordinates_not_means.md) — 판별문=**이 문장을 지우면 무엇을 고칠지 모르나(→준다) 어떻게 고칠지 모르나(→안 준다)**. 주는 것=불변식·**적용 좌표 전수(grep)**·**재현 데이터 지목**·RED 문장·새 조합 목록 / 안 주는 것=구조·API·알고리즘. 🔑R33 은 불변식만 받고 **적용 범위를 스스로 정해** 삭제/취소만 고치고 수정/수정 충돌을 새로 만들었다. ⚠️**지시 구체화가 안 듣는 쪽**=선재 결함(08-05 실측 자기유발 3 vs 선재 10) — 그건 범위 문제이니 분리 가능성을 개발책임자께 올릴 것
- [🚨 표준 워크플로우 유일 진실원 (2026-08-05 현행)](feedback_canonical_workflow.md) — 머지게이트=①실 사용자 경로 재현 결함 0 ②CI green(exact SHA) ③라이브QA 실서버 실행. 🚨**적대리뷰는 오직 도달성만**(검증품질 폐지) · 🆕🚨**2차는 SOL 자기완결 사이클** — SOL 검증→**SOL 이 fix 지시서 작성**→LUNA fix→SOL 재검증→**결함 0 일 때만 PM 보고**(PM 이 매 홉 끼면 라운드가 배로 늘었다). PM 재검증은 도장이 아니라 **게이트 — 이상하면 다음 라운드로 되돌린다**. 검증 완료 후 **머지 판단을 받는다**. 지시 주체는 SOL(좌표 제공은 1차 라운드 몫) · 리뷰어/구현자 분리 유지 · **커밋은 라운드마다**(게시·보고만 사이클 끝으로). 1차=OPUS 발견 / SONNET5 대조. `gpt-5.6-sol`/`gpt-5.6-luna`
