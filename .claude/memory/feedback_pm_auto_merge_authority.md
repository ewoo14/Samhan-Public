...
## 🚨 2026-07-29 — **문서 전용 변경은 PR 을 열지 않는다**

> 개발책임자: *"아 문서만 변경할거면 PR 열지마"*

**제품 코드 변경 0건**(`docs/**` · `.claude/memory/**` · 핸드오프 · dev-report) 이면 **브랜치·PR 없이 main 에 바로 push** 한다.

### 왜

PR 은 **적대검증·CI 게이트를 태우기 위한 장치**다. 문서에는 태울 게이트가 없다. 그런데 PR 을 열면:
- CI 40여 체크가 돌고(문서 가드 몇 개 말고는 무의미)
- GitGuardian 오탐 판정 같은 부수 작업이 붙고
- 열린 PR 수가 늘어 **실제 작업 트랙이 안 보인다**

2026-07-29 세션에서 PM 이 문서 PR 을 **4건**(#973 #975 #982 #983) 열었다. 전부 제품 코드 0건이었고 전부 그냥 머지됐다.

### 규칙

| 변경 내용 | 방식 |
|---|---|
| **문서·메모리 전용**(제품 코드 0건) | **main 직접 push.** PR 없음 |
| 제품 코드가 한 줄이라도 포함 | 기존대로 브랜치 + PR + 게이트 |

⚠️ **판별은 `git diff --name-only` 로** 한다. *"문서 위주"* 가 아니라 **제품 코드가 정확히 0건**일 때만 직접 push 다. 스크립트(`scripts/**`)·CI 설정(`.github/**`)·테스트는 **제품 코드**다.

🔑 이 조항은 기존 *"main 직접 docs/memory push 는 별개 가드"* 를 **대체**한다.
