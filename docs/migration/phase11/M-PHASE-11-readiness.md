# M-PHASE-11-readiness — Phase 11 진입 plan (AWS migration cutover, 기존 Phase 10 → Phase 11 renumber)

> **Phase 번호 renumber 적용 (D-P10-05, 사용자 결정 2026-05-07)** — 기존 Phase 10 (AWS migration cutover) 은 Phase 11 으로 이동, 신규 Phase 10 = arologis-service (배차 마이크로서비스). 본 문서는 기존 phase10 readiness 의 모든 plan / 일정을 Phase 11 컨텍스트로 보존 (모든 § 본문은 동일한 AWS cutover 작업 분해를 의미하며, 단지 phase 번호만 11 으로 의도).

본 문서는 Phase 10 (arologis-service) 완료 시점에서 Phase 11 (AWS 마이그레이션 + Migration Service + 운영 안정화) 진입을 위한 전제 조건, 작업 분해, 슬라이스 roadmap, 가드를 정리한다. Phase 9 회고 (`docs/dev-reports/phase9-retrospective.md`) + Phase 11 dry-run plan (`docs/migration/phase11/M-AWS-MIGRATION-DRY-RUN.md`) 과 짝을 이룬다.
