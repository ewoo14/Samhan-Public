...
> **PR-H4c 변경** — INBOUND 슬립 + lot 등록의 모든 필드는 변경될 때마다 audit log 가 누적되며, [02. 출고 처리](02-출고-처리.md) § 2-9 와 동일한 audit overlay 가 입고 화면에도 적용됩니다. 사용자 명시 패턴 (`docs/uiux/phase12/H2-audit-overlay.md`) inventory 도메인 적용 — slip 화면과 시각 1:1 동일.

### 6-1-1. 입고 슬립 헤더 "수정 N회" badge

INBOUND 슬립 상세 화면 우측 상단에 **수정 N회** badge 표시 (출고 슬립과 동일 임계 색상).

### 6-1-2. 필드별 audit overlay (수량 / lot / 적요)

각 필드 우측에 가장 최근 변경:

```
입고 수량         95 EA                ~~100 EA~~          ● 김창고 13:32 (검수 5개 불량)
lot 번호          LOT-2026-05-09-001    (시드 그대로)
적요              발주 100, 실수령 95   (시드)
```

### 6-1-3. lot inbound POSTED FULLY_LOCKED (한국 회계 무결성)

lot 등록이 완료 (`POSTED` — 회계 전기 완료) 된 직후부터는 직접 수정 불가:

```
[잠금 banner]
이 lot 은 완전 잠금 상태입니다 — MASTER 에게 문의하세요.
회계 전기 완료 — 한국 일반기업회계기준 보존 의무 (별도 회계 정정 분개로만 수정).
```

POSTED 후 차이 발견 시 = 별도 inventory adjust (재고 조정) 슬립 + 자동 정정 분개 (110 / 401 reverse) 진행.

### 6-1-4. SUBMITTED LOCKED_REQUIRES_APPROVAL (검수 직전)

`SUBMITTED` (검수 대기) 상태 = LOCKED_REQUIRES_APPROVAL — 수정 요청 워크플로우 (PR-H3 시드 패턴):

```
[잠금 banner]
이 입고는 잠금 상태입니다 — 수정 요청을 보내주세요.
검수 대기 단계 = 회계 전기 직전. 변경 시 권한자 (MANAGER / MASTER) 승인 필요.
...
WAREHOUSE 가 입고 검수 중 수량을 수정하면 동시에 화면을 보고 있던 SALES (영업) / MANAGER 의 입고 화면이 1초 안에 자동 갱신.

```
[우측 상단 toast]
실시간 변경 감지 — 0.8초
김창고 님이 입고 수량 을 "95 EA" 으로 수정했습니다. (rev #2 — 검수 5개 불량)
```

### 6-1-6. UUID 비공개 + 비즈니스 식별자 노출

화면 어디에도 UUID 노출 0건. 비즈니스 식별자만 표시:

- 슬립번호 (`S-2026-NNNNN`) / lot 번호 (`LOT-2026-05-09-001`) / 품목코드 (P-2026-0001) / 창고 코드 (`HQ-001`)
