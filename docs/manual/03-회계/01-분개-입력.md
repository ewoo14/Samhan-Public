...
## 6-1. 수정 이력 보기 (Phase 12 step-4c, PR-H4c FE rollout)

> **PR-H4c 변경** — 분개의 모든 필드(적요/금액/계정 코드/거래처 코드 등)는 변경될 때마다 audit log 가 누적되며, JournalDetailPage 화면에서 "취소선 + 수정자 색상 + 수정자 이름" overlay 로 한눈에 확인할 수 있습니다. 사용자 명시 패턴 (`docs/uiux/phase12/H2-audit-overlay.md`) 의 분개 도메인 적용 — slip 화면과 시각 1:1 동일.

### 6-1-1. 분개 헤더 "수정 N회" badge

JournalDetailPage 우측 상단에 **수정 N회** badge 표시 (PR-H2 시드와 동일 임계 색상).

| 횟수 | 색상 | 의미 |
| ---: | --- | --- |
| 0회 | (badge 숨김) | 신규 입력 후 수정 없음 |
| 1~4회 | 회색 | 정상 범위 |
| 5~9회 | 노랑 | 다중 수정 주의 환기 |
| 10회+ | 빨강 | 비정상적 다수 수정 — 한국 회계 감사 시 점검 권장 |

### 6-1-2. 필드별 audit overlay (적요 / 금액 / 계정 코드)

각 필드 우측에 가장 최근 변경이 inline 으로 표시됩니다.

```
적요    현금 매출 입금        ~~현금 입금~~        ● 이회계 13:32
금액    1,500,000             ~~1,200,000~~        ● 이회계 13:31
계정    100100 · 현금          (시드 그대로)
```

### 6-1-3. POSTED FULLY_LOCKED — 정정 분개로만 수정

POSTED 상태 분개는 한국 일반기업회계기준 보존 의무에 따라 직접 수정 불가:

```
[잠금 banner]
이 분개는 완전 잠금 상태입니다 — 정정 분개로만 수정 가능합니다.
한국 일반기업회계기준 보존 의무 (POSTED 후 별도 정정 분개 절차).

[정정 분개 작성]  ← 클릭 시 신규 분개 화면 (반대 부호 라인)
```

CLOSED (월/연 마감) 상태는 더 엄격: MASTER 도 차단 + 감사인 동석 별도 절차 의무.

### 6-1-4. UUID 비공개 가드

화면 어디에도 분개/사용자 UUID 가 직접 노출되지 않음:

- 화면 표시: 분개번호 (`J-2026-0512`) + actorName 풀네임 (`이회계`)
- 색상 식별: dot 색상 (`userIdToColor(actorId)` hash)
- 한국 계정 코드 (`100100` 현금 / `220000` 부가세예수금 / `401000` 상품매출) 노출

### 6-1-5. 1초 SSE 실시간 sync (multi-context)

ACCOUNTANT 가 분개를 수정하면 동시에 화면을 보고 있던 MANAGER / MASTER 의 JournalDetailPage 가 1초 안에 자동 갱신 (audit overlay row 추가 + toast 알림).

```
[우측 상단 toast]
실시간 변경 감지 — 0.8초
이회계 님이 적요 를 "현금 매출 입금" 으로 수정했습니다. (rev #2)
```

> 본 동작은 PR-H4a `shared-realtime` BE 모듈 + PR-H4b 13 service rollout + 본 PR-H4c FE 통합으로 50+ page 일관 동작.
...
- PR-H4c QA scenarios + 작동 캡처: `docs/qa/phase-12-step-4c-fe-audit-overlay-rollout/scenarios.md`
