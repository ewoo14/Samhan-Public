...
## 6-1. 수정 이력 보기 + ACTIVE 잠금 (Phase 12 step-4c, PR-H4c FE rollout)

> **PR-H4c 변경** — P0-6 fix 후 정식 PartnerDetailPage 도입 시 본 절의 audit overlay + ACTIVE 잠금 정책이 자동 적용됩니다. 본 절은 시드 패턴 (PR-H2 `02-출고-처리.md` § 2-9 + PR-H3 § 2-10) 의 거래처 도메인 적용 — slip 화면과 시각 1:1 동일.

### 6-1-1. 거래처 헤더 "수정 N회" badge

PartnerDetailPage / PartnerListPage 우측 상단에 **수정 N회** badge 표시 (시드 임계 색상 0/1~4/5~9/10+ 동일).

### 6-1-2. 필드별 audit overlay (사업자명 / 대표자 / 연락처 / 주소)

각 필드 우측에 가장 최근 변경이 inline:

```
사업자명           삼한전자(주)         ~~삼한전자~~        ● 오영업 13:32
연락처             02-1234-5678         ~~02-1111-2222~~    ● 박관리 13:30
대표자             김삼한               (시드 그대로)
주소               서울 강남구 ...      (시드 그대로)
사업자등록번호     123-45-67890         (시드 그대로 — 변경 시 별도 절차)
```

### 6-1-3. ACTIVE LOCKED_REQUIRES_APPROVAL — 수정 요청 워크플로우

거래처 status 가 `ACTIVE` (정식 거래 중) 인 경우 직접 수정 불가:

```
[잠금 banner]
이 거래처는 잠금 상태입니다 — 수정 요청을 보내주세요.
ACTIVE 상태 = 활성 거래처 (영업 / 회계 의존). 변경 시 권한자 승인 필요.

[+ 수정 요청]  ← 클릭 시 SlipEditRequestDialog 패턴 (사유 50자 권장)
```

워크플로우 (PR-H3 시드와 동일):

1. SALES (또는 작성자) 가 `[+ 수정 요청]` 클릭 → dialog 열림
2. 사유 입력 (예: "거래처 사업자명 변경 요청 — 법인 전환")
3. 권한자 (MANAGER / MASTER) 가 알림 수신 → approve
4. SALES 의 mutation 1회 한정 통과 (consume approval)
5. 두 번째 mutation 시 다시 잠금 (재요청 필요)

### 6-1-4. SUSPENDED / INACTIVE FULLY_LOCKED

`SUSPENDED` (정지) 또는 `INACTIVE` (softDelete) 거래처는 MASTER 만 수정 가능.

```
[잠금 banner]
이 거래처는 완전 잠금 상태입니다 — MASTER 에게 문의하세요.
정지/비활성 거래처는 MASTER 만 별도 절차로 수정 가능.
```
...
- 거래처코드 (`P-2026-0001`) / 사업자등록번호 (`123-45-67890`) / 사업자명 / 대표자명
- actorName 풀네임 + 색상 dot

### 6-1-6. 1초 SSE 실시간 sync (multi-context)

SALES 가 거래처 정보를 수정하면 동시에 화면을 보고 있던 MANAGER / 다른 SALES 의 PartnerDetailPage 가 1초 안에 자동 갱신.

```
[우측 상단 toast]
실시간 변경 감지 — 0.8초
박관리 님이 연락처 를 "02-1234-5678" 으로 수정했습니다. (rev #2)
```

### 6-1-7. 복원 dropdown (MANAGER+)

각 audit row 우측의 ▾ 드롭다운에서 복원 가능 — MANAGER + MASTER 권한만 노출.
...
복원 = 새 revision 추가 (이전 revision 영구 보존).
