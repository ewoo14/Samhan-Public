...
**잔여(각 슬라이스 확정):**
- **결재 필수성**: 전표 발행/확정에 결재 승인 **필수**(승인 완료 후 유효) vs **선택**.
- **lifecycle 관계**: 출고 기존 lifecycle(`accept`→`dispatcherUserId` 자동 채움)과 권한 그룹 명시 결재의 관계 — **대체**(자동 채움 폐기, 결재라인 설정 권한 그룹 결재로 일원화) 유력하나 A4 확정.
- **전표↔결재선 연계 방식** / **서명 동결 위치** / **회계 재사용 경로**.
