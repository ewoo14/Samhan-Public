...
## 결정 확정 (2026-06-23 개발책임자)
- ✅ **G 어음 상태 = 보유(BOARDING)/추심(COLLECTING)/결제완료(SETTLED)/부도(DISHONORED) 4상태**. 배서양도·어음할인은 후속.
- ✅ **H CSV = 범용 컬럼 매핑 UI** — 업로드 시 일자/입금/출금/잔액/적요/상대 컬럼을 사용자가 매핑(어느 은행 CSV든 수용, 매핑 프로파일 저장 재사용).
- 거래처원장 전기 시 상대계정: 적요/거래처 규칙 기반 제안 + 수동 확정(H-3에서 상세).
