...
- **실 admin VIEW 화면 (FE route + GET)**: ~~`ecount.mig14.cash-list`~~(슬2 PR #520 제거), `ecount.mig14.order-list`, ~~`ecount.mig14.aging-snapshot`~~(슬1 PR #518 제거), `ecount.mig14.ledger`, `ecount.mig.ops-dashboard` — `clients/desktop/src/renderer/routes/index.tsx` 에 `<PermissionGuard ... action="view">` route 존재. 이카운트 raw 의 우리 DB parity 조회용 읽기 화면. (현행 잔여 = order-list / ledger / ops-dashboard 3종. cash-list·aging-snapshot 은 네이티브 편입 에픽에서 silo 폐기 — 아래 표 참조.)
...
| ~~ecount.mig14.cash-list~~ **(제거됨 — 슬2 PR #520)** | (구 admin VIEW) 지출결의서·입금보고서 목록 | ⛔ GET `/api/v1/accounting/cash-disbursements`+`/cash-receipts` 와 FE route(`CashDisbursementListPage`·`CashReceiptListPage`·`CashTransactionList`)·page-code 폐기. 현금 자료는 MIG-9 가 이미 네이티브 journals 에 편입 → **분개장 `/accounting/journals` + 입금매칭 `/accounting/deposit-match` + 원장** 으로 노출. (`CashDisbursement`/`CashReceipt` 엔티티·`cash_*` 테이블·Mig7 transform·Mig9 cash-journal 은 lineage 로 유지) | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
...
| ~~ecount.mig14.aging-snapshot~~ **(제거됨 — 슬1 PR #518)** | (구 admin VIEW+갱신) 거래처 잔액 스냅샷 | ⛔ GET `/api/v1/accounting/aging-snapshot`·refresh endpoint 와 FE route(`PartnerAgingSnapshotPage`)·DTO·page-code 폐기. 거래처 잔액은 네이티브 보고서 `/accounting/reports/partner-aging`(POSTED journals 110/201 직접 집계, 재무 보고서 메뉴) 로 대체. (MV `partner_aging_snapshot` + `Mig9AgingSnapshotRefreshService` 는 lineage 로 유지) | ❌ | ❌ | ❌ | ❌ | ❌ |
...
- **실 admin VIEW 화면 (FE route + GET)**: ~~mig14.cash-list~~(슬2 PR #520 제거), mig14.order-list, ~~mig14.aging-snapshot~~(슬1 PR #518 제거), mig14.ledger, mig.ops-dashboard — 원래 **5 PageCode** 였으나 네이티브 편입 에픽에서 cash-list·aging-snapshot 2종을 silo 폐기 → **현행 3종**(order-list / ledger / ops-dashboard). 거래처 잔액 = 네이티브 partner-aging 보고서, 현금 자료 = 네이티브 분개장/입금매칭/원장으로 대체.
