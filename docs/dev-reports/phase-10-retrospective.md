...
6. **GitGuardian dev-only 비밀번호 false positive 처리** (`feedback_gitguardian_false_positive.md`): PR #100 fix `4a8152c` 시점 dev-only test 비밀번호 (samhan_dev_pw / samhan!2026 / dev-internal-token-change-me) 검출. 옵션 B (dashboard false positive mark) 표준화. git history rewrite + admin 강행 머지 회피.
