...
## 0-A. 🆕 08-16 03:30 스냅샷 — 밤새 진행분

### 머지 완료 (야간)

```text
#1224  단톡방 거래처코드 106 연결 / 4 모호 / 2 미매칭      9994c248f
#1226  없는 URL 이 500 이 아니라 404                        54fe5c535
(그 전) #1221 · #1218 · #1219
```
...
| PR | 도달 결함 | CI | 남은 것 |
|---|---|---|---|
| #1222 금액 콤마·키보드 | **0** (7경로 라이브QA) | 확인 중 | CI green 확인 후 머지 |
| #1223 전환 흐름·사이드바 | **0** (8역할 216조합) | 16건 fix 중 | `접수/완료` 축 테스트 갱신 |
| #1225 가입고 XLSX | fix 후 r3 검증 중 | pending | 멱등 실동작 확인 |
| #1162 자격 로테이션 | 0 (범위 밖 1건 별도) | fix 후 확인 중 | 🚩 **머지 보류** ↓ |
