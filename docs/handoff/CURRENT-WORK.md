...
**main = `420395191`** · 열린 PR **1건**(#982 문서) · 열린 이슈 **17건**

## 이 세션 전체 성과

| 구간 | 머지 | 이슈 CLOSED |
|---|---|---|
| 새벽 | #969 #974 #968 #967 #957 #958 | #964 #965 #963 #851 #863 |
| 아침 | **#981**(#935 아로로지스 버전) · **#980**(#896 슬3 가격정합) | **#928**(실측 결과 이미 완료) |
| 대기 | #982(문서 — #874 정찰) | |

### 막은 금액
`−7,600,000원`(분기관 77→1) · `6,710,000원`(GHP 누락) · `−2,057,000원`(T형 분기관) · **`60,000원/대`**(방진가대 30HP) · **`4,235원/대`**(360 리모컨) · **대당 `90,000원`**(인상기준일 4~6월) · 시트 sync lost update

---

## 🔴 회사PC 에서 먼저 볼 것 — **개발책임자 결정 3건 대기**

### ① 일마감 판정 구조 (#874)
현재 일마감이 **거래처·문서 경계 없이 품명을 합산**한다. 이 구조에서는 약정DC 를 고를 수 없고 **다른 문서의 구성품을 세트로 잘못 짝지을 수 있다.**
⟹ **거래처·문서별 행 분리** vs **기존 품명 요약에 보수적 집계**. 계산 추가 전 **선행 결정**이다.

### ② 🚨 주문 확정 단가 드리프트 — **출시 차단 후보**
> `partner-order-service` 가 옵션 플래그 **6종을 전부 `false`** 로 보내고 **품목 고정DC 도 전달하지 않아** FE 와 서버 확정 단가가 달라질 가능성

#874 정찰이 범위 밖에서 발견했다. **새 이슈 등록 안 함** — 정찰 관찰이라 **사실 확인이 선행**돼야 한다. 회사PC 에서 **가장 먼저 실측**할 것.

### ③ #977 축소 착수 여부
24개 GAS 전수 재대조 중 **금액에 닿는 프로젝트만**(주문·견적·일마감) 골라 개시 전에 돌릴지.
...
| # | 상태 |
|---|---|
| **#874** | 정찰·기획 완료(841줄, PR #982). **결정 ①② 후 구현** |
| **#875** | 구형 baseline 처리. 자격 블로커는 해소됨(오늘 시트 sync 실행 성공) |

📌 **#896 은 슬4(설정화)가 남아 열려 있다.** 개발책임자 확인 — *"리모컨 구성품 변경이나 방진가대 수량 동기화도 모두 해당 메뉴에서 편집이 가능한 범위에 들어가는 게 맞아."*
⟹ 슬4 = **부속 모델 매핑 17곳 · 제품군 판별 정규식 · 리모컨 선택 · 용량 구간**을 칩 설정으로. 선례는 `price_change_schedule`(테이블 + 관리 API + FE 화면)이 이미 그 형태다.

## 개시 후 — 15건
#935(설치본 검증만 남음) #922 #910 #903 #901 #895 #894 #883⏸️ #845 #827 #826 #825 #977 #978 · #896 슬4

---

## 🚩 이 세션 후반이 남긴 것

### 골든 정답지를 바꿀 때 — **oracle 은 레거시 정본**
PR #980 에서 골든에 `AR-EH05` 가 새로 나타났고, PM 이 원인을 규명했다:
```text
실 DB      AR-KH05 → COMMERCIAL_MULTI 노출 O (18,150)  ⟹ 실 운영에선 스킵 안 됨. 과다청구
골든 fixture  fixtures.js:44  h('AR-KH05')  홈에만
              fixtures.js:122 c('AR-EH05')  상업에만   ⟹ 상업 360 케이스에서 골든이 리모컨을 아예 기록 안 함
```
**금액 오류(실 운영)와 fixture 커버리지 공백(골든)이 겹쳐 있었다.** 🚩 **골든 합성 상업 카탈로그 보강**은 이월.

### 이슈가 안 줄어드는 구조
*"많이 머지했는데 이슈가 별로 안 닫혔다"* 지적 — 실제로 **결함 수정 루프는 백로그를 줄이지 않는다**. 적대검증이 찾은 결함으로 새 이슈가 생기고 그걸 닫는 순환이라, **신기능 백로그를 직접 깎아야** 준다. 이 세션에서 PM 이 이월을 이슈화(#977 #978)해 **백로그를 오히려 +2** 했다.

### PM 실수 (누적 6건, 전부 리뷰 축이 잡음)
cwd 착오 **2회** · `REAL_QA_ALLOW_UNTRACKED` 오판 · 변경 줄 수 정정문에서 같은 오류 재현 · **라운드 상한제 미발동** · **수단 지시로 lost update 유발** · **결정 대기하며 진행 가능한 것까지 멈춤**(#980 ①②)

🔑 마지막 건 — *"선택지 나열하며 기다리는 것 자체가 병목"*. **물어볼 것과 진행할 것을 가른 뒤 진행할 것은 즉시 착수**한다.

## 🚩 환경 함정 (추가분)
1. **`git worktree add` 시 브랜치명 중복** — 다른 워크트리가 쓰는 브랜치는 삭제도 안 된다. 새 이름을 쓸 것
2. **병렬 트랙은 파일 경로로 격리** — 브리핑에 *"`clients/web/order-app/**` 등은 다른 트랙이 수정 중이니 쓰지 말 것"* 을 명시해야 충돌이 안 난다
3. `estimate-app` 은 **로컬 `node_modules` 없음** → `npm ci` 선행. 안 하면 견적 골든 RED 를 CI 에서야 본다
4. 커밋 메시지에 큰따옴표가 있으면 `-m` 이 깨진다 → `Write` → `git commit -F`

---

# 🏁 2026-07-29 새벽 (집PC) 마감 — **열린 PR 0** · 머지 6건 · 이슈 5 CLOSED

> 🚩 시작 절차: `git pull` → `.\scripts\sync-claude-memory.ps1` → 이 문단.

**main = `ebf9737c9`** · **열린 PR 0건** · 열린 이슈 **21 → 16**

## 머지 6건

| PR | 이슈 | 핵심 |
|---|---|---|
| #969 | **#964 CLOSED** | 예외모드 누출(BLOCKING) 등 5건 — 세션에 남은 `REAL_QA_ALLOW_UNTRACKED` 가 공식 전체 실행까지 오염 |
| #974 | — | **GAS 정본 26개 갱신 + 자격 마스킹 게이트화** (44파일) |
| #968 | **#965 CLOSED** | 손상 `<img>` 가 용지에서 글자를 덮던 **4번째 표면** |
| #967 | **#963 CLOSED** | 수량 계열 6군 + **GHP 6,710,000원 누락** |
| #957 | **#851·#863 CLOSED** | **판정 실패-개방 6건** 10사본 fail-closed 전환 |
| #958 | — | 값 열거 탈피 + **lost update 해소**(V25 순서 토큰). `#896` 은 슬3 남아 열어 둠 |

### 오늘 막은 금액
`−7,600,000원`(구형 snapshot 복원 시 분기관 77→1) · `6,710,000원`(GHP 저감장치 주문서 누락) · `−2,057,000원`(상업 T형 분기관) · 시트 sync lost update.

## 🚨 개발책임자 지시로 바뀐 운영 규칙 (상시 적용)

> *"워크플로우가 너무 무거워서 자꾸 무한 루프를 돌 것 같으면, 개선방향을 자꾸 마련해야해. PM이 자율진행으로 해당 트랙에 대한 방향을 조정하고 **결함없이 머지가 되면 돼**."*

→ [[feedback_pm_adjusts_workflow_weight_per_track]] 등재. **정밀**(금액·회계·권한·무결성=캐논 유지) / **신속**(UI·문서·하네스·CI·관측 = 1라운드+CI green+라이브QA 1회) 로 PM 이 트랙마다 배분한다. 애매하면 **금액에 닿는가**로 가른다.

## 🚩 이 세션이 남긴 방법론 3건 (메모리 등재)

1. **[[feedback_narrow_briefing_completes_wide_times_out]]** — #957 SOL 이 넓은 브리핑으로 **7200초 타임아웃 2회·산출물 0**(4시간). **단일 항목**으로 좁히자 완주하고 **6건**. MCP codex 는 강제 종료 시 산출물이 하나도 안 남는다 ⟹ 범위가 크면 "덜 찾는" 게 아니라 **아무것도 못 받는다**
2. **[[feedback_pm_means_instruction_creates_defect]]** — *"잠금이 외부 응답 구간을 덮지 마라"* 는 불변식이 아니라 **수단 지시**였고 lost update 를 PM 이 만들었다. 판별법 = **"이 문장이 구현자의 선택지를 하나로 좁히는가?"**
3. **[[feedback_pm_adjusts_workflow_weight_per_track]]** — 위 규칙

## 🚩 PM 실수 5건 (전부 리뷰 축이 잡음 — 릴레이 금지의 근거)

| | 내용 |
|---|---|
| 1 | **cwd 착오** — 워크트리를 main 으로 착각해 `172/172` 오보. 절대경로로 재측정해 `176/172` 확인 |
| 2 | `REAL_QA_ALLOW_UNTRACKED` 를 *"명시적 opt-out 이라 결함 아님"* 으로 오판 → SOL 이 **BLOCKING** 으로 잡음 |
| 3 | 변경 줄 수 **정정문에서 같은 오류 재현**(`21=20+1`·`5=3+2`) |
| 4 | **라운드 상한제 미발동** — *"fix 가 네 번 연속 새 표면"* 을 PR 에 직접 적고도 바운드 옵션을 안 올림. 개발책임자가 *"라운드 상한제가 있을텐데"* 로 지적 |
| 5 | **cwd 착오 2회차** — #958 검증을 main 에서 돌려 `44/505`. 워크트리에서 `58/614` 재확인 |

🔑 **1·5 가 같은 원인**이다. 워크트리 6개를 동시에 다루는 구조에서 반복된다 — **워크트리 작업은 절대경로로 진입하고 실행 전 HEAD 를 찍어 대상 SHA 를 확인할 것.**

## 🔴 개발책임자 판단 대기 — **새 이슈 등록 허락 1건**

GAS 정본 갱신(#974)이 드러낸 **미반영 가격 변경**. 개발책임자 확인 결과 *"가격이 변동되었으니 감안하라"* → **결함이 아니라 반영 대상**이다.

| 항목 | 라이브 GAS | 제품 | 영향 |
|---|---|---|---|
| 주문서 인상기준일 | `2026-07-01` | `2026-04-01`(DB `V22` 시드) | 납품희망일 4~6월 주문 **대당 90,000원** |
| 일마감 인상기준일 | `20260701` | `2026-04-01`(price_history) | 마감액 불변, **`확인` 플래그 오탐**(45%→66%) |
| 상업 구성품 기준가 | 납품가 우선 | 출고가 우선 | 현재 카탈로그에선 미도달(가려져 있음) |
| 360 실내기 리모컨 | `AR-EH05` | `AR-KH05` | 4,235원/대 |
| 냉방전용 30HP 방진가대 | S2중(160,000) | S2대(220,000) | 60,000원/대 |
| I형 유연호스 단가 | 8,000 | 7,000 | 1,000원/EA |

⚠️ **estimate-app 은 납품가, order-app 은 출고가**를 씁니다 — 두 앱이 서로 다른 규칙입니다.
⚠️ 조사 담당이 *"재다운로드 보고서 §4 요약이 거래처 주문서에서 크게 축소돼 있었다"* 고 밝혔습니다 — **나머지 24개 GAS 프로젝트 요약도 그대로 신뢰하기 어렵습니다.**
📄 상세: `scratchpad/gas-drift-product-impact.md`(세션 한정) · PR #967 머지 코멘트에 표 보존

## 🚩 이월 (신속 경로 바운드로 후속 이관)

- **#957** — 경로 표기별 10사본 판정표 · 다른 OS 컨테이너(Linux/pwsh7) · `net use` 매핑 드라이브 · 선재 `operational-validation.ps1` 항목4 `Join-Path` 배열
- **#968** — 다섯 번째 표면 확인 안 함(마지막 fix 이후 SOL 미실시)
- **#967** — 일곱 번째 계열 확인 안 함 · 같은 함수의 **조용한 skip 4곳**(구현자 보고, 범위 밖)
- **#958** — `syncTab` 롤백 시 `lastKnownRowHash` 잔존(정적 관찰, 미확정) · 게이트웨이 경유 · GUI

## ✅ 로컬 실데이터 (2026-07-28 복구 완료 — 아래 문단 참조)

품목 **1,220** · 거래처 **7,260** · 노출 **865** · lookup **28/32/6** · 사양 **9,008** · 노션 4종 재추출.
⚠️ **`docker compose up --force-recreate` 하면 SA 키가 사라집니다**(컨테이너 `/etc/samhan/sa-key.json` 에 직접 설치).
🔴 **이카운트 품목 임포트만 미완** — 회사PC 에만 있는 `품목관계` export 필요(아래 문단에 절차·명령 그대로).

## 🚩 환경 함정 (이 세션 실측)

1. **워크트리 `remove` 가 파일 잠금으로 실패**한다(`Permission denied`/`Invalid argument`). 브랜치 삭제는 되므로 디렉터리는 나중에 prune
2. **`estimate-app` 은 로컬에 `node_modules` 가 없어 `jest` 가 아예 안 돈다.** order-app 만 확인하고 넘기면 견적 골든 RED 를 CI 에서야 발견한다
3. **codex 콘텐츠 필터** — *"자격 없이 실제 경로를 태울 수 있다"* 류 문장이 자격 우회로 읽혀 **3회 연속 차단**됐다. `application.yml` 설정 키를 그대로 쓰는 중립 서술로 바꾸면 통과
4. **셸 인용** — 커밋 메시지에 큰따옴표가 있으면 `-m` 이 깨진다. 반드시 `Write` → `git commit -F`
5. **CI 플래키** — vitest 는 테스트 파일이 늘면 워커 분배가 바뀌어 잠재 상태 누출이 드러난다. 모듈 레벨 `vi.fn()` 은 `afterEach` 에서 **`mockReset()`** 해야 한다(`mockClear` 로는 `*Once` 큐가 안 지워짐). 해소 확인은 **셔플 시드 여러 개**로

---

## 📌 2026-07-28 (집PC) — 로컬 실데이터 복구 완료 · **회사PC 에서 마저 할 일 1건**
...
**main = `28c991307`** (메모리 PR #970 머지분 포함)

### 🚨 가장 먼저 — 라운드 교대를 어기고 있었다

캐논 5단계 *"3~4 를 도달가능 0 까지 반복"* 은 **3 = OPUS 라운드 ↔ 4 = CODEX SOL 라운드 교대**인데, 실제로는 `OPUS → fix → OPUS → fix` 로 돌아 **5트랙 전부 SOL 0회**였다. **집PC 자기감사가 지적한 결손(머지 6건 전건)을 회사PC 에서 그대로 반복**한 것이다.

바로잡아 5트랙 전부에 SOL 을 걸었고 **1회차에서만 도달 가능 결함 11건**이 나왔다.

> 📌 메모리 등재 완료 — [[feedback_sol_stage_starvation]] (PR #970 머지). **fix 커밋 직후 "다음은 무엇인가" 의 기본 답은 SOL 이다.** 트랙 상태를 볼 때 `SOL 실행 횟수`를 항상 함께 셀 것.

### 트랙 현황

| PR | 브랜치 / 워크트리 | SOL 발견 | 상태 |
|---|---|---|---|
| **#957** (#851) | `chore/851-fe-be-contract-gate` · `.claude/worktrees/851-gate` | 미실시 | 🔴 **CI RED 2건** fix 중 → 그 뒤 SOL |
| **#958** (#896) | `feat/896-s2-quantity-sync-schema` · `896-s2` | **3건** | LUNA fix 중 |
| **#967** (#963) | `fix/963-legacy-gas-quantity-loss` · `963-qty` | **4건** | LUNA fix 완료(코드 유효) · **라이브QA 증거 재작성 중** |
| **#968** (#965) | `fix/965-document-image-decodability` · `965-imgvalid` | **2건** | LUNA fix 커밋 `dc9a9e990` · 경고 가독성 후속 중 |
| **#969** (#964) | `chore/964-real-qa-testmatch-scope` · `964-testmatch` | **2건** | LUNA fix 중 |

### SOL 이 찾은 것 중 특히 무거운 것

- **#969** — **게이트 자체가 실 CLI 에서 미작동.** `npx playwright test --config playwright.real-qa.config.ts --list` 가 `Total: 548 tests in 172 files / EXIT=0` 으로 통과했다. **단위 테스트 29/29 는 GREEN 이었다** — 합성 argv 로 함수를 직접 부르는 세계와 playwright 가 config 를 로드하는 세계가 달랐다. 게다가 `^1.59.1` 범위에서 `npm update` 만 해도(→1.62.0) `createFileMatcherFromArguments is not a function` 으로 **명시 파일 실행이 전멸하고 막아야 할 전체 실행만 살아남는다**.
- **#958** — **동시 생성 요청 2건이 모두 201** 로 커밋돼 활성 규칙 그래프에 **실제 순환**이 저장된다(`is_two_cycle = t`). DB 강제층 제거(개발책임자 승인 축소)의 실측 결과. 🚫 **트리거 복원은 금지** — Java 층에서 해결하도록 브리핑함(#830 advisory lock 선례 참고만 제시).
- **#967** — 상업멀티 4건, 전부 금액 영향(−160,000 / +698,500 / −181,632 / 838,200원 차이 미표시).

### 🚩 PM 판단 오류 4건이 실행으로 드러났다

| | 무엇을 |
|---|---|
| #967 | R2 에서 *"상업 잠금은 시각 표시 없음(홈만 게이트)"* 이라 적고 넘긴 비대칭이 **결함 4건 전부**가 됨 |
| #958 | *"의도적으로 성깁니다"* 며 통과시킨 카테고리 가드가 **무변경 저장까지 409 과차단** |
| #958 | 두 라운드 연속 *"정적 관찰, 미확정"* 으로 이월한 시트 동기화 우회가 **실제로 도달 가능** |
| #969 | R2 fix 검증 때 29/29 를 재실행했지만 **게이트를 실 CLI 로 두드려 보지 않음** |

### 🚩 반복 패턴 2가지 (다음 세션에서 그대로 쓸 것)

1. **"미검증" 이라 정직하게 적은 항목이 그다음 CI RED 가 된다.** #957 에서 두 라운드 연속: R4 *"Linux 실측 안 함"* → Linux RED, R5 *"pwsh 7 미검증"* → pwsh RED. ⟹ fix 브리핑에 **"미검증을 적을 거면 왜 검증이 불가능한지도 함께 쓰라"** 를 넣는다.
2. **변경 줄 수 오보** — 메모리 등재([[feedback_changed_line_count_misreport]]). 오늘도 #957 R5 보고가 `qa/playwright/utils/screenshot.ts` **1파일을 누락**했고 PM 재계수로 잡았다.

### 🚩 도구 함정 (이 세션 실측)

- **`jq` 가 이 Git Bash 에 없다.** `gh pr view --json X -q '...'`(gh 내장 jq)를 쓸 것. `| jq` 를 쓴 Monitor 가 **조용히 무동작**했다.
- **codex 서브에이전트가 스크래치패드 경로를 오타냈다** — `0da21b3f-c1a8-4df5-…` / `0a21b3f-…` 라는 **형제 세션 디렉토리를 새로 만들어** 산출물을 거기 남겼다. 브리핑에 경로를 주고 **저장 후 `ls` 출력을 보고에 넣게** 할 것.
- **LUNA 가 거짓 전제로 fixture 를 썼다** — *"`commercialMulti` 0건"* 이라 보고했으나 PM 이 같은 부트스트랩을 직접 호출하니 **404건**이었다(`AM080AXVHHH1` 이 첫 항목). **산출물 검증은 릴레이 금지**가 이래서 필요하다.

---

# 🏁 회사PC 세션 마감 (2026-07-28 저녁) — 여기가 **재개 지점**

**main = `0c2ce5686`** · 머지 2건(#970 메모리 · #971 핸드오프, 둘 다 문서 전용·제품 코드 0)

## 🚨 집PC 에서 가장 먼저 할 일

**돌고 있던 codex 작업 3건은 세션과 함께 죽습니다 — 재디스패치해야 합니다.** 무엇을 지시했는지 아래에 그대로 남깁니다.

| PR | 상태 | 다음 행동 |
|---|---|---|
| **#957** | `10d57ea4c` · 실제 CI 실패 **0**(GitGuardian 오탐만) | 🔁 **SOL 라운드 재디스패치**(1회차, 유일 미실시). 워크트리 `851-gate` |
| **#958** | `236a3ac18` · CI **38/38** | ✅ SOL+fix+결함③ 라이브 완결. **재수렴 라운드** 착수(발견 OPUS + 대조 SONNET5) |
| **#967** | `a44449d3c` · CI 진행 중 | 🔁 **선재 결함 2건 fix 재디스패치**(아래 상세). 재수렴 2각도는 **완료** |
| **#968** | `d57f14c31` · CI **40/40** · 워크트리에 미커밋 6건 | 🔁 **재설계 fix 재디스패치**(아래 상세). 재수렴 2각도 **완료** |
| **#969** | `da2ca3ade` · CI **39/39** | **재수렴 라운드** 착수(2각도). SOL+fix 2회 완료 |

전 워크트리 **미커밋 0 · 미푸시 0**(단 `965-imgvalid` 는 진행 중 작업분 6건). 메인 트리 클린.

### 🔁 재디스패치 1 — #957 CODEX SOL 라운드 (1회차)
워크트리 `.claude/worktrees/851-gate` @ `10d57ea4c`. 이 PR 은 **2차 적대검증을 아직 한 번도 안 돌았습니다.**
핵심 지시 사항:
- **10사본 × 입력집합 판정표를 SOL 이 직접 다시** 만들 것(평범 / UNC localhost·127.0.0.1·컴퓨터명·**자기 LAN IP** / UNC 슬래시·혼합 / **subst** / **크로스드라이브 junction** / `-ProjectRoot` 타 체크아웃)
- 🚨 **"조용한 무력화" sweep** — 직전 CI fix 가 밝힌 결함 B 가 *"P/Invoke 예외를 `$ErrorActionPreference=Continue` 가 삼켜 판정이 미정의로 새고 D-2 가 exit 0 통과"* 였다. **판정 함수 실패 시 ALLOW 로 새는 지점**이 또 있는지
- **플랫폼 × 에디션 격자** — PS 5.1 / pwsh 7(Win) / pwsh 7(**Linux, Docker `mcr.microsoft.com/powershell`**) / Node(Win·Linux) / Python / sh
- **과차단 0** 확인(`203.0.113.77` UNC 는 통과해야 함)
- 미완 이월: **`net use` 매핑 드라이브 별도 RED/GREEN**, Linux N-1(git blob), 라인엔딩 수정 후 완전격리 전체 스위트
- 선재 `operational-validation.ps1` 항목4 `Join-Path` 배열 버그 — **도달성 판정만**(고치지 말 것)
- 851-gate 에 `node_modules` 없음 → `NODE_PATH` 또는 임시 junction(검증 후 제거)

### 🔁 재디스패치 2 — #967 선재 결함 2건 fix (CODEX LUNA)
워크트리 `.claude/worktrees/963-qty` @ `a44449d3c`. **PM 이 흡수 판정을 이미 내렸습니다**(PR 코멘트에 근거 게시).

| | 결함 | 금액 | 원인 |
|---|---|---|---|
| ① | 상업멀티 옵션 5종이 **품목 검색 한 번에 전부 기본값 리셋**(검색어 지워도 원복 안 됨, 저장→복원도 동일) | **+202,830원** | `renderComm()`(`index.html:4497`)이 매 렌더마다 `renderCommOptions()` 호출 → 첫 줄 `box.innerHTML=''` + 하드코딩 기본값. 필터·검색·`applySnapshot` 전부 이 경로. **홈은 `renderHome()` 이 옵션을 재생성하지 않아 무사** |
| ② | 상업 **T형 분기관** 수동 수량 미잠금 → 재계산에 소실, 파란 굵은 글씨도 안 뜸 | **−2,057,000원** | `COMM_MANUAL_*` 이 `PANEL/HOSE/REMOTE/PUMP/BASE` **5종뿐, BRANCH 없음**(홈은 `PANEL/HOSE/REMOTE/BRANCH/FOOT`). `commManualSetForRow`(`:2911`) → null → `recomputeCommDerived()` 가드(5636~5639)에 분기관 없어 무조건 덮음 |

- **둘 다 `origin/main` 에서 동일 수치 재현** = 선재. 별도 dev 서버(5502)에 main 을 띄워 확증했다
- ②는 **"홈만 게이트" 계열의 5번째** — fix 커밋이 스스로 *"홈 파생 5계열(판넬·호스·리모컨·**분기관**·발통)"* 을 열거하고 분기관만 미이식
- ①을 흡수하는 이유: 이 PR 이 편집한 `renderComm()` 안쪽이고, **새 이슈 등록에 사전 허락이 필요**해 분리가 오히려 막힌 길
- 불변식: *"상업멀티가 홈멀티와 같은 규칙을 따른다"* + *"렌더가 사용자의 옵션 선택을 지우지 않는다"*. **수단은 지시하지 말 것**

### 🔁 재디스패치 3 — #968 경고 표시 재설계 (CODEX LUNA)
워크트리 `.claude/worktrees/965-imgvalid` @ `d57f14c31`(미커밋 6건 존재 — 상태 확인 후 이어가거나 폐기 판단).

🚨 **이 트랙은 fix 가 세 번 연속 새 표면을 만들었습니다.**
| 라운드 | 고친 것 | 남긴 것 |
|---|---|---|
| R1 fix | 가로 확산 | **세로 넘침** |
| `dc9a9e990` | 세로 넘침 | **문구를 못 읽음** |
| `d57f14c31` | 문구 잘림 | **문구가 아예 사라짐 + 흐름 분기 겹침 잔존** |

**원인 = 세 요구가 작은 박스에서 동시 충족 불가** — ①자기 영역 이탈 금지 ②읽을 수 있을 것 ③박스는 임의로 작을 수 있음(5%×3% = 26×2.7px). **박스 안에서 답을 찾는 한 계속 진동합니다.**

발견 각도가 잡은 도달 가능 결함 3건:
1. **[HIGH]** `⚠` 축약이 **박스 크기와 무관** — `DocumentRenderer.tsx:289` 분기가 `geometry === undefined` 단 하나이고 크기 조건이 없다. 편집기 기본이 좌표 배치(`useTemplateDraft.ts:171`)라 **실사용 형태 전부**. **525.69×90.70px 박스도 `⚠` 하나**인데 같은 문장이 흐름 배치 `525.69×16.80px` 한 줄에 들어간다. 터치는 `any-hover:none` → `title` 도달 불가, 탭해도 `dialogs=0`. `5%×3%` 에서는 `⚠` 조차 `overflow:hidden` 으로 사라져 **2.7px 붉은 실선**만 남고, 불투명 배경이 `<img>` alt 까지 덮는다
2. **[MEDIUM]** 흐름 배치 경고가 **HEADER/FOOTER 밴드에서 형제 좌표 글자를 덮음**(`onTop=40/40`). `compileApprovalDocument` 가 BODY 만 geometry 로 거르고 헤더/푸터는 안 거른다. **제품 `좌표 해제` 버튼(`ElementInspector.tsx:316`) 클릭 2번**으로 도달
3. **[LOW]** 큰 미지원 형식(BMP 67,854B)은 여전히 "용량" 안내 → **줄인 뒤에야**(30,054B) 진짜 이유

**개정 불변식**(PM 이 이미 하달, 수단 미지시):
> **4′** 깨진 이미지가 있을 때 사용자는 ①있다는 사실 ②**어느 요소인지** ③무엇을 해야 하는지를 **박스 크기와 입력 방식(마우스 없는 터치 포함)에 무관하게** 알 수 있다
> **1′** 경고가 다른 요소의 글자를 덮지 않는다 — 좌표·흐름 양쪽, **머리말·본문·맺음말 전 밴드**
> **2′** 거부 이유가 실제 사유와 일치한다 — **파일 크기와 무관하게**
> **3′** 인쇄물에는 경고가 나오지 않는다(불변)

🔑 **4′ 은 박스 안에서 만족 불가** — 정보를 박스 밖으로 빼야 크기 의존성이 사라진다. 함께 해소될 것: *"저장이 차단되는데 어느 이미지인지 안 알려준다"*(GRID 는 깨진 이미지 4개) — **PM 이 게이트로 판정**.

**함께 지시한 문서 정정 4건**(미완):
1. `2ef26713a` 의 **"FE 239/239 · 27개 파일"** 이 틀림 → 실측 **233건 / 26파일**(PM 재계수: `git ls-tree` 26 · 신규 2파일 vitest 5건 · 238−5=233). 커밋 메시지는 못 고치니 **dev-report §8.8 에 정정 각주**
2. **"거부 경로 5항목"** → 실제 코드 분기 **4개**(`ElementInspector.tsx:249-281`, 0바이트·텍스트위장이 형식불일치 분기 공유). 숨은 분기 없음 → "입력 5종 / 분기 4개" 로
3. **"환경에 `pdftotext` 없다"** → PowerShell 한정. **Git Bash `/mingw64/bin/pdftotext`(xpdf 4.06) 실존**. 앞으로 PDF 검증은 두 도구 교차
4. **dev-report 가 R1 fix 이후 미갱신** — SOL 라운드·SOL fix·가독성 fix 절 신설

## ✅ 이 세션에서 끝난 것

### 캐논 교대 결손 시정 → SOL 1회차만으로 **도달가능 11건**
`OPUS→fix→OPUS→fix` 로 돌아 5트랙 전부 SOL 0회였다. 바로잡자 #967 **4건** · #958 **3건** · #969 **2건** · #968 **2건**이 나왔다. → 메모리 등재 [[feedback_sol_stage_starvation]] (#970 머지)

### 머지된 PR 사후 점검 — **결함 0 · 증거 무결성 위반 0**
개발책임자 지시로 *"결함 0"* 선언됐던 **#956 · #952 · #950** 을 `origin/main` 전용 워크트리에서 재감사. 메모리에 남은 두 우려(**#956 `@Primary` Javadoc 거짓** · **#952 자기 게이트 0**)는 **둘 다 머지 전에 R2/R3 가 스스로 교정**해 둔 것이었다. 런타임 실측 — 형제 5개 점유 중에도 outbox `outbox-1` 821ms tick, `@Async`→`task-1`, 명암비 **8.3101:1**(fallback 아닌 실제 토큰), Enter/Space 가 `aria-pressed` 실제 왕복, mock 652건 후 `docs/qa` 5,778 무변경. 결과는 세 PR 에 게시.

### 각도 분리가 **실제로 다른 것을 잡았다**
- **대조(SONNET5)** — #968 의 `239/239·27파일` 오보(→233/26) · #967 의 **스크린샷 8장이 캡션과 다른 화면** · 수치 25건 전건 대조
- **발견(OPUS)** — `⚠` 축약이 크기 조건 없이 전 좌표 배치에 적용 · 터치 도달 불가 · #967 의 선재 2건
서로 겹치지 않았다.

## 🚩 이 세션에서 실측된 함정 (집PC 에서 그대로 쓸 것)

1. 🚨 **`jq` 가 이 Git Bash 에 없다.** `gh pr view --json X -q '...'`(gh 내장) 을 쓸 것. `| jq` 쓴 Monitor 가 **조용히 무동작**했다.
2. 🚨 **`| tail` 뒤의 `$?` 는 `tail` 의 종료코드다.** 파이프 없이 재거나 `PIPESTATUS`. PM 이 이걸로 `EXIT=0` 오독했다.
3. 🚨 **gradle `BUILD SUCCESSFUL in 3s` = 캐시(미실행).** `--rerun-tasks --no-build-cache` 로 강제. PM 이 담당 산출물을 릴레이할 뻔했다.
4. 🚨 **공유 Docker 이미지가 낡았다** — `dashboard`·`notification` **2026-06-08**, `dc-config` 06-18, `auth` 07-07, `api-gateway` 07-14, `partner-order`·`user`·`inventory` 07-24. **BE 를 바꾸는 트랙은 라이브QA 전 재빌드 필수.**
5. 🚨 **codex 가 스크래치패드 경로를 오타내 형제 세션 디렉토리를 만든다**(`…4df5…`, `…0a21b3f…`). 저장 후 **`ls` 출력 제출**을 의무화해도 **내용은 확인 못 한다** → **캡처 후 내용을 프로그램으로 검증**하게 할 것(대상 행 값·computed style·`getBoundingClientRect()`).
6. 🚨 **"자격이 없어서 못 했다" 는 보고를 그대로 믿지 말 것 — 오늘 두 번 다 사실이 아니었다.** #827 은 clasp CLI 버전 문제였고 자격은 이미 있었다. #958 은 `GOOGLE_SHEETS_ENDPOINT_OVERRIDE`(`GoogleSheetsClient.java:244-247`)로 Google 자격 없이 실 경로 검증이 가능했고 **SOL 은 이미 그렇게 하고 있었다**.
7. `pdftotext` 는 **PowerShell 엔 없고 Git Bash `/mingw64/bin` 엔 있다**.
8. **junction/symlink 는 `rm -rf` 금지**(대상 내용까지 지울 위험). PowerShell `(Get-Item p).Delete()` 로.
9. **admin-hr-guard.spec.ts 5건이 `localhost:5173` 하드코딩** — 지정 포트를 무시한다(별개 관찰).

## 📌 미해결 결정
없음. 개발책임자 지시(사후 점검·GAS 8,000원·자율 진행)는 전부 반영·완료.

---

## 🏁 2026-07-28 (집PC 마감) — **머지 13건 · 이슈 4 CLOSED / 3 신규 · 🚨 워크플로우 자기감사 시정**
