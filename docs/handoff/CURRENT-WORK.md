...
**다음 한 걸음** — SOL 라이브QA(`kh6sdknho`) 결과를 먼저 보고, LUNA fix 라운드.
🔑 이 트랙의 핵심 함정: **API·DB 축은 `266,800` 인데 화면 축은 `316,800`** 이었습니다.
계산 함수를 직접 부르지 말고 **화면이 실제로 내는 요청**으로 재십시오.
...
⚠️ 구현자가 **slip-service 전량을 못 돌렸고**(timeout) **desktop typecheck 도 못 했습니다**.
CI 가 그 미검증 구간에서 터진 것입니다.

🔑 **이 슬라이스의 핵심 불변식 = "아무것도 되돌리지 않는다".** S1 은 **판정만** 합니다.
판정 API 전후로 전표 상태·재고·배차 연결이 하나도 바뀌면 안 됩니다.

**다음 한 걸음** — SOL 라이브QA(`k9fs2opmr`) 결과 확인 → LUNA fix 라운드로 CI 3건 해소.
🚨 `#1142` 되돌림 **권한·범위·연결 처리·이력**은 **결정 대기** — S1 을 넘어가지 마십시오.

---

### 🟡 `fix/1092-split-unified-list` — **구현 중 (PR 미개설)**
...
**다음 한 걸음** — LUNA(`kljeur05p`) 산출물 커밋 → **PR 개설**(연관 `#1092`) → 적대검증 라운드.
