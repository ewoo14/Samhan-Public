...
- [~] **E3** 입금보고서 에픽 — 설계탐색+**S1 완결**. 설계=`docs/superpowers/plans/2026-07-03-e3-deposit-report-epic-design-exploration.md`. 개발책임자 결정 확정(상태 DRAFT→CONFIRMED→CANCELLED·③계정 기본 103/110+변경가능·mutation {id} UUID path-var·무결성 preconfirm 완료).
  ...
  - 🔜 **다음(회사PC 재개)=S2**: 라이브 POSTED 분개+역분개 — `postAutoJournal(sourceType=CASH_RECEIPT)` 배선(CONFIRMED 시 분개생성·수정 시 `autoReverse` 재게시)·계정매핑(기본 103/110·CashReceipt debit/credit 사용)·`Mig9CashJournalService` 라이브 레퍼런스·`partner_aging_snapshot` refresh. → S3(통장연계 `markReflected` 승격·N건→입금보고서) → S4(FE 목록/작성폼/BankTransactionPage 다중선택/coedit·목업 `docs/design/mig-14-admin-ui/02_cash_receipt_list_mock.md`).
  - **backlog(S1 재검)**: snapshot restore partner 재조회(S4 coedit)·Mig9 stub 현실성·request partnerCode/bizNo 필터 422(S4 UX)·verification.md stale·findBySlipNo dead·ci.yml narrow gate drift·⚠️**로컬 auth_db V79 checksum mismatch — 회사PC도 flyway repair or auth_db fresh 필요**([[feedback_applied_migration_immutable]]).
