...
- [~] **E3** 입금보고서 에픽 — ✅ **정찰+설계탐색 문서** `docs/superpowers/plans/2026-07-03-e3-deposit-report-epic-design-exploration.md`(2026-07-03). Journal 완성형(라이브 역분개·postAutoJournal·원장불변 코드화)·BankTransaction CSV/CODEF 실피드+행매칭·`Mig9CashJournalService`=브릿지③ 배치 레퍼런스·born-live 3엔티티 온보딩됨. 4다리(②CashReceipt 상태/채번/CRUD→③POSTED분개+역분개→①통장연계 markReflected→④FE 다중선택/작성폼/coedit). ⚠️**③ 계정과목 매핑=무결성 도메인→개발책임자 brainstorming+preconfirm 후 착수**([[feedback_integrity_domain_policy_preconfirm]], 구현 PR 미개설). morning 결정 5건(CashReceipt 상태 라이프사이클·③계정매핑·통장 카디널리티·편집정책·FE 다중선택 UX).
