...
### ⚠️ 개발책임자 회신 대기 (갱신)
① gross(total_*) 유효발생분 축소 여부(현=원장 유량) ② ~~거래처 열 BE enrich~~ → **이슈 #713 으로 승격**(회신 시 슬라이스 착수) ③ #688 단가변동 DRAFT 방향 ④ **🆕 마감기간 역분개 정책**(#710 코멘트 4876916057, 무결성 preconfirm): **A안=409 차단**(원분개가 마감기간이면 취소/계정변경 재게시도 차단 — S2 확정/수정 409 와 대칭·보수적) vs **B안=열린 기간 일자 역분개 허용**(later-period reversal — 마감 재무제표 불변 유지+취소 업무 지속·리포트 영향 검토 필요). **결정 즉시 fix PR 착수.**

### 📌 이후 순번
① (④결정 후) **마감 역분개 가드 fix PR**(가드+닫힌 기간 취소/재게시 IT) ② **E3 S3 착수 전 소형 정비 fix PR**(표시명 "현금입금"→"입금보고서" 3곳 + override 계정 IT false-green 보강 — 정책 무관 즉시 가능) ③ **E3 S3(통장연계)**: markReflected 라이브 승격·BankTransaction N건 선택→입금보고서 1건 생성(생성 전 거래처 매칭 강제·journalId 확보 후 reflected) ④ S4(FE 목록/작성폼/BankTransactionPage 다중선택/coedit·목업 `docs/design/mig-14-admin-ui/02_cash_receipt_list_mock.md`). backlog 는 S2·#712·#711 dev-report 참조(+mock.ts 시산표 주석=E3 S4 동봉).
