...
> 2. **종합견적서 에픽 스코핑**(다음 주력): 견적서(기본/세트상세)=스냅샷 저장+웹 재로드. GAS 양식 **754KB(~19k줄) 대형**. **📋 PM 야간 스코핑 제안 작성 = `docs/superpowers/specs/2026-06-15-comprehensive-quotation-epic.md`** (GAS 구조 분석 + 우리 도메인 데이터모델 + sub-slice 4단계[마스터DB→폭발/단가엔진→데스크톱UI+2양식+진입버그fix→스냅샷/웹재로드] + 결정포인트 6건). **결정 필요(스펙 §6)**: 스냅샷 동결정책(완전동결 권장 vs 구조만) / 세트모델·옵션delta 형태 / 배분비율(가정6:4·상업4:6) / VAT·카드수수료 금액계산 / 조합비초과 경고vs차단 / 마스터데이터 시드1회vs동기화. → 결정 후 sub-slice1(마스터DB)부터 착수.
