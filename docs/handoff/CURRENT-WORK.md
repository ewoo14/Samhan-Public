...
## 🟢 현재 상태 (2026-06-17 최신) — 에픽 #18 완결(#494/#495) → **방향 전환: 기초품목↔견적품목 분리 에픽**(견적품목 관리 메뉴 신설, 설계 박제 완료, 슬1 착수 예정)
...
>   - **슬라이스 계획**: 슬1=견적품목 관리 메뉴/화면 신설(기초품목 선택추가 + 노출M:N·표시순서 이관, 기초품목=등록전용화) → 슬2=세트구성(bundle_component)+구성품정렬(#495)+판넬/리모컨/자재 옵션 이관 → 슬3=변동DC+G1 카탈로그 DB(견적품목 도메인 내, Java FORMULA read 상업멀티 useK2 378 parity 포함).
