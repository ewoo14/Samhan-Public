...
### 실행 순서 (PM 자율)
1. 회계체인: **#823 ✅ 머지**(main `3fee4b699`·PR #849)→**전표 거래처 필수화 ✅ 머지**(main `78cb759d9`·PR #853)→**#850 배분 과할당 버그 ◀ 다음 착수**→**#825 슬5**  2. #825 슬6(쪽지칩)·슬7(주문병합)  3. **#845 DS-3**→DS-4  4. #824(품목 VAT)·**#848 ✅ 머지 완료**(main `0c8210df0`·PR #852)  5. chore 배치(#831·#832·#838·#839·#828)  6. AC 후속(#834·836·837·840·842·843)  7. 🔴 **Google 자격 후** #827·#773  8. Phase11 #830 · cutover-defer #826
...
>
> **✅ 전표 거래처 필수화 — 생명주기 전이 가드 · 머지 완료**(main `78cb759d9`·PR #853·2026-07-19): committed(SENT+) 전이 시 `partner_id` 필수 불변식(`status ∈ REQUIRED_PARTNER_STATUSES=전상태−{DRAFT,SAVED,CANCELED} ⟹ partner_id≠null`). **3중 도메인 가드**(`send()`·`restoreFromSnapshot()` 표준+협업·forward 8전이 `requirePartnerForCommitted()`)+**발행 fail-closed**(`resolveCommittedPartnerId` FOUND+partnerId 만·NOT_FOUND/5xx/SKIPPED/empty 차단)+FE preflight+**보정 엔드포인트**(`POST /internal/slips/backfill-committed-partners`·멱등·dry-run·audit). **컬럼 NOT NULL 비채택**(DRAFT null 정당). 워크플로우: OPUS 기획→SOL 검수 3R GO→LUNA 구현→OPUS R1(fixture 9 CI RED→fix·라이브QA 14→0)→SOL R2(forward 미가드·보정 아티팩트·send HTTP IT·docs→fix)→재수렴(OPUS 수렴+SOL docs)→재수렴 fix(reject 순서·README×2·FE 강화·CRLF)→OPUS 델타검증 **0수렴**→CI 33/33→머지. 라이브QA 음성(400)/양성(SENT)/보정(§8→0). **별건 #854**(outbox self-invocation·pre-existing). **다음 = #850 배분 과할당 버그**(회계체인).
