...
- **일자**: 2026-07-12 · **작성**: PM(Opus) 심화 정찰 기반 · **상태**: 🟡 개발책임자 결정 대기(구현 전)
...
### D1 (최우선) — "재계산"의 의미
- **ⓐ 레거시식 할인율 재검증 워크시트** (감사 리포트·**마감 금액 불변**·`확인` 플래그) ← *레거시 실제 동작·회계 원장 수정금지 원칙과 정합*
- ⓑ 인상 전 가격으로 what-if 총액 재산출(**마감 금액 변경**) ← *원장/마감 불변 원칙과 충돌*
- ⓒ 마감 SUM referent 전환
